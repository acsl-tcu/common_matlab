...
%
%  ★重要: 揺れの固有周波数 ωn はケーブル長で大きく変わる.
%    実機/対象に合わせて cableL・mass・loadmass を必ず設定すること.
...
agent.parameter.set("cableL",  0.46);
...
ylabel("\tau_y [N·m]"); title("励振入力（総トルク）");
subplot(2,1,2); plot(T, rad2deg(Y), "LineWidth",1); grid on;
ylabel("\theta [deg]"); xlabel("time [s]"); title("牽引物の揺れ角");
