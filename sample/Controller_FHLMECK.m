function Controller= Controller_FHLMECK(time)
% 階層型線形化コントローラの設定
...
Ac4 = diag([1,1,1],1);
Bc4 = [0;0;0;1];
Controller.F1=lqrd(Ac2,Bc2,diag([100,1]),[0.1],time.dt);                                % 
Controller.F2=lqrd(Ac4,Bc4,diag([100,10,10,1]),[0.01],time.dt); % xdiag([100,10,10,1])
Controller.F3=lqrd(Ac4,Bc4,diag([100,10,10,1]),[0.01],time.dt); % ydiag([100,10,10,1])
Controller.F4=lqrd(Ac2,Bc2,diag([100,10]),[0.1],time.dt);                       % ヨー角
...
syms sz1 [2 1] real
syms sF1 [1 2] real
[Ad1,Bd1,~,~] = ssdata(c2d(ss(Ac2,Bc2,[1,0],[0]),time.dt));  % 状態空間表現から係数行列A, Bを取得
