...
% controller property をController classのインスタンス配列として定義
    ...
    if class(agent.plant)~="DRONE_EXP_MODEL"
        ...
        Controller.F2 = lqrd(A6,B6,diag([1000000,2000,1000,1,0.001,0.001]),0.1,dt);%より制御周期速くなると0.008とかが良くなると思う．
        Controller.F3 = lqrd(A6,B6,diag([1000000,2000,1000,1,0.001,0.001]),0.1,dt);
        Controller.F4 = lqrd([0 1;0 0],[0;1],diag([10,1]),[1],dt);                %yawの位置、速度、100,1,1
