...
% controller property をController classのインスタンス配列として定義
    ...
    if class(agent.plant)~="DRONE_EXP_MODEL"
        ...
        %
        Controller.F1 = lqrd(A2,B2,diag([150,10]),0.01,dt);                        % z方向サブシステム, h1^(i) i = 0~1
        Controller.F2 = lqrd(A6,B6,diag([4000,1500,1000,200,10,10]),0.1,dt);   % x方向サブシステム, h2^(i) i = 0~5
        Controller.F3 = lqrd(A6,B6,diag([4000,1500,1000,200,10,10]),0.1,dt);   % y方向サブシステム, h3^(i) i = 0~5
        Controller.F4 = lqrd(A2,B2,diag([10,1]),0.01,dt);
    else
        % exp用
        ...
        Controller.F2 = lqrd(A6,B6,diag([1000,350,100,10,1,0.1]),0.01,dt);%より制御周期速くなると0.008とかが良くなると思う．
        Controller.F3 = lqrd(A6,B6,diag([1000,350,100,10,1,0.1]),0.01,dt);
        Controller.F4 = lqrd([0 1;0 0],[0;1],diag([10,1]),[1],dt);                %yawの位置、速度、100,1,1
