...
%% common param
    ...
    %%　実験用　重み
    Controller.weight.P = 1.5*diag([300;300;500]);    % 位置　10,20刻み  20;1;30
    Controller.weight.Q = 1*diag([1000;1000;1000]);    % 速度  10,20刻み  30;20;10
    Controller.weight.V = 1*diag([100;100;100]); % 15良い気がする
    Controller.weight.W = diag([100;100;100]);  % 姿勢角，角速度　1,2刻み 
    Controller.weight.R = diag([200; 200; 200; 200]); % 入力
    Controller.weight.RP = 0.1*diag([100; 100; 100; 100]);  % 1ステップ前の入力との差    0*(無効化)
