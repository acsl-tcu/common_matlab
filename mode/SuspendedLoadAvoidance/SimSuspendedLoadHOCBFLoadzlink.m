...
agent.controller.set_function_class("hlc_suspended", HLC_SUSPENDED_LOAD_HOCBF_LOAD_Z_LINK_NEW(agent, Controller_HL_Suspended_Load(dt, agent)));
% % =================================================================
...
% 新しい回避対応版クラスをインスタンス化
% mov = DRAW_SUSPENDED_LOAD_HOCBF_Z_LINK(app.logger, ...
%     "target", 1, ...
%     "self", app.agent(1), ...
%     "rl", actual_rl);
% mov.animation(app.logger,"target", 1,"self", app.agent(1), "rl", actual_rl); % 表示開始
% actual_a_sys = app.agent(1).controller.result.a_sys; % 長軸
% actual_b_sys = app.agent(1).controller.result.b_sys;               % 短軸
% mov = DRAW_SUSPENDED_LOAD_HOCBF_Z_LINK_EL(app.logger, ...
%     "target", 1, ...
%     "self", app.agent(1), ...
%     "a_sys", actual_a_sys, ...
%     "b_sys", actual_b_sys);
...
%     "a_sys", actual_a_sys, ...
%     "b_sys", actual_b_sys);
actual_lambda_list = app.agent(1).controller.result.lambda_list; % 位置比率配列 (例: [0.0, 0.5, 1.0])
actual_rl_list     = app.agent(1).controller.result.rl_list;     % 各球体の半径配列 (例: [0.20, 0.15, 0.35])

% 2. 描画オブジェクトの初期化
mov = DRAW_SUSPENDED_LOAD_HOCBF_Z_LINK_MLT(app.logger, ...
    "target", 1, ...
    "self", app.agent(1), ...
    "lambda_list", actual_lambda_list, ...
    "rl_list", actual_rl_list);
