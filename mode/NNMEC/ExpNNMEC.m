...
% ref = TIME_VARYING_REFERENCE(agent,{"gen_ref_saddle",{"freq",10, "center",[0;0.5;takeoff_zd], "radius",[0,0,0]}, 4}); % point_hover
...
% refpoints = {struct("f",center, "g",[center(1:2)+1;takeoff_zd+1]), 10};
...
agent.reference.set_function_class("go2ini", GO_TO_FLIGHT_INITIAL_STATE_REFERENCE(agent));
...
agent.cha_allocation.a.reference = ["takeoff","go2ini"];
agent.cha_allocation.t.reference = ["takeoff","go2ini"];
...
app.logger.plot({{1, "w1", "e"},{1, "input2", ""}},"fig_num",1, "phase",phase, "Fontsize",FS, "Linewidth",LW);
