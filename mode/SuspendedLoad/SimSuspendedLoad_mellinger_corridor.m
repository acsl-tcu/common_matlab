...
% appにプロパティを追加せず、単にコントローラへセットする
replanner_instance = REPLANNING_MELLINGER_CORRIDOR_FLATNESS_V2(agent, nominal_ref, replan_opts);
...
% agent の中からリプランナを探し出して DRAW_SUSPENDED_LOAD_CORRIDOR に渡す
target_ref = [];
try
    func_list = app.agent(1).reference.func;
    if iscell(func_list)
        for i = 1:length(func_list)
            if contains(class(func_list{i}), 'MELLINGER')
                target_ref = func_list{i};
                break;
            end
        end
    end
...
mov = DRAW_SUSPENDED_LOAD_CORRIDOR(app.logger, ...
    "target", 1, ...
    "self", app.agent(1), ...
    "replanner", target_ref);
