...
function post(app)
    ...
    for k = 1:4
        ...
        if k >= 2; ylim([-1.2, 1.2]); yline([-1, 1], 'k:'); end
    end
    xlabel('Time [s]');
    ...
    for k = 1:4
        subplot(4, 1, k);
        plot(t_span, u_diff(k, :), 'm-', 'LineWidth', 1.5);
        ylabel(titles_du{k}); grid on;
        if k == 1; title('2. Modification Amount: u_{CBF} - u_{nom}'); end
    end
    xlabel('Time [s]');
    ...
    try
        m_true = app.agent(1).parameter.get("loadmass");
    catch
        try
            m_true = app.agent(1).parameter.loadmass;
        catch
            m_true = 0.14;
        end
    end
    yline(m_true, 'k--', 'True Mass', 'LineWidth', 1.5);
