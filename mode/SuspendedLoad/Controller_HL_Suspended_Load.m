...
% controller property をController classのインスタンス配列として定義
    ...
    if class(agent.plant)~="DRONE_EXP_MODEL"
        ...
        % 2026/04/15 Yamato 田中チューニング %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
              Controller.F1 = lqrd([0 1;0 0],[0;1],diag([100,10]),[1],dt);             %位置z、速度z
        Controller.F2 = lqrd(A6,B6,diag([5000,4000,4000,10,0.001,0.001]),0.01,dt);%より制御周期速くなると0.008とかが良くなると思う．
        Controller.F3 = lqrd(A6,B6,diag([5000,4000,4000,10,0.001,0.001]),0.01,dt);
        Controller.F4 = lqrd([0 1;0 0],[0;1],diag([10,1]),[1],dt);  
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
