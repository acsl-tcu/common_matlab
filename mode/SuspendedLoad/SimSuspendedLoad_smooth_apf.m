...
% 【最新版】滑らかな APF (人工ポテンシャル法) リプランナ設定 (ベースライン)
replan_opts = struct();
replan_opts.k_att      = 2.0;
replan_opts.k_rep      = 15.0;
replan_opts.d_obs_max  = 5.0;
replan_opts.force_max  = 5.0;
replan_opts.w_filt     = 2.0;

agent.reference.set_function_class("timevarying", ...
    REPLANNING_SMOOTH_APF(agent, nominal_ref, replan_opts));
