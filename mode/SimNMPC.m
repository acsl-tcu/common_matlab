...
agent.reference.time_varying = TIME_VARYING_REFERENCE(agent,{"gen_ref_saddle",{"freq",7,"orig",[0;0;0.6],"size",[1,1,0.2]},"HL"});
 agent.controller = NMPC(agent,Controller_NMPC(dt,agent));
%run("ExpBase");
...
app.logger.plot({1, "v", "er"}, "fig_num", 3,"xrange",[app.time.ts,app.time.te], "linewidth", 2.5, ...
 ...
 app.logger.plot({1, "p1-p2-p3", "er"},"fig_num", 4,"phase",'tfl', "color",0, "linewidth", 2.5, ...
    "fontsize", 14);
 % app.agent.animation(app.logger, ...
 %    "target",1, ...
 %    "fig_num",999, ...
 %    "mpc_test",false, ...
 %    "mp4",true, ...
