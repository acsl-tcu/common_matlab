...
%     % 楕円体障害物・原形コア・システム側ダイナミクス保護球の残像防止・完全可視化
% 
%     properties
%         replanner_ref
%         h_obs_core      = [] % 障害物原形
%         h_obs_hard_ell  = [] % ハード楕円体
%         h_obs_soft_ell  = [] % ソフトマージン帯
%         h_load_margin   = [] % 荷物保護バブル (水色)
%         h_drone_margin  = [] % ドローン保護バブル (水色)
%     end
% 
%     methods
...
% 
%             % 水色バブルの頂点
%             X_L = sp_x * r_load + pL(1);
%             Y_L = sp_y * r_load + pL(2);
%             Z_L = sp_z * r_load + pL(3);
% 
%             X_Q = sp_x * r_drone + pQ(1);
%             Y_Q = sp_y * r_drone + pQ(2);
%             Z_Q = sp_z * r_drone + pQ(3);
% 
%             hold(obj.ax, 'on');
% 
%             % --- 障害物原形 (赤・不透明) ---
%             if isempty(obj.h_obs_core) || ~isvalid(obj.h_obs_core)
%                 obj.h_obs_core = obj.create_core_geometry(c, R_obs, radii);
%             end
% 
%             % --- ハード楕円体 (オレンジ・半透明) ---
%             if isempty(obj.h_obs_hard_ell) || ~isvalid(obj.h_obs_hard_ell)
%                 obj.h_obs_hard_ell = surf(obj.ax, X_hard, Y_hard, Z_hard, ...
%                     'FaceColor', [1.0 0.5 0.0], 'FaceAlpha', 0.20, ...
%                     'EdgeColor', 'none', 'DisplayName', 'ハード楕円体境界');
%             else
%                 set(obj.h_obs_hard_ell, 'XData', X_hard, 'YData', Y_hard, 'ZData', Z_hard);
%             end
% 
%             % --- ソフトマージン帯 (黄色メッシュ) ---
%             if isempty(obj.h_obs_soft_ell) || ~isvalid(obj.h_obs_soft_ell)
%                 obj.h_obs_soft_ell = mesh(obj.ax, X_soft, Y_soft, Z_soft, ...
%                     'EdgeColor', [0.9 0.8 0.1], 'FaceColor', [1.0 0.9 0.2], ...
%                     'FaceAlpha', 0.05, 'LineStyle', ':', 'DisplayName', 'ソフトマージン帯');
%             else
%                 set(obj.h_obs_soft_ell, 'XData', X_soft, 'YData', Y_soft, 'ZData', Z_soft);
%             end
% 
%             % --- 荷物保護バブル (水色・毎フレーム確実に削除＆再生成して残像を完全撲滅) ---
%             if ~isempty(obj.h_load_margin) && isvalid(obj.h_load_margin)
%                 delete(obj.h_load_margin);
%             end
%             obj.h_load_margin = surf(obj.ax, X_L, Y_L, Z_L, ...
%                 'FaceColor', [0.0 0.8 1.0], 'FaceAlpha', 0.35, 'EdgeColor', 'none');
% 
%             % --- 機体保護バブル (水色・毎フレーム確実に削除＆再生成して残像を完全撲滅) ---
%             if ~isempty(obj.h_drone_margin) && isvalid(obj.h_drone_margin)
%                 delete(obj.h_drone_margin);
%             end
%             obj.h_drone_margin = surf(obj.ax, X_Q, Y_Q, Z_Q, ...
