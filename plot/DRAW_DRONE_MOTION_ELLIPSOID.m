...
classdef DRAW_DRONE_MOTION_ELLIPSOID
    ...
    methods
        function obj = DRAW_DRONE_MOTION_ELLIPSOID(logger, varargin)
            param = struct(varargin{:});
            if ~isfield(param, "target")
                param.target = 1;
            end
        ...
        function draw_obstacles(obj)
            ...
            if exist('ENVIRONMENT_OBSTACLE_HOCBF_ELLIPSOID', 'file') == 2
                ...
                for i = 1:length(obs_list)
                    ...
                    % 2. 安全マージン外接球または楕円体の描画 (CBFが認識しているバリア境界)
                      if isfield(obs, 'A_mat') && ~isempty(obs.A_mat)
                          [x_m, y_m, z_m] = sphere(30);
                          points = [x_m(:), y_m(:), z_m(:)]';
                          T_mat = real(sqrtm(inv(obs.A_mat))) * obs.c_scale;
                          points_new = T_mat * points;
                          x_m = reshape(points_new(1,:) + obs.p_obs(1), size(x_m));
                          y_m = reshape(points_new(2,:) + obs.p_obs(2), size(y_m));
                          z_m = reshape(points_new(3,:) + obs.p_obs(3), size(z_m));
                      elseif isfield(obs, 'p_obs') && isfield(obs, 'r_obs_margin')
                          [x_m, y_m, z_m] = sphere(30);
