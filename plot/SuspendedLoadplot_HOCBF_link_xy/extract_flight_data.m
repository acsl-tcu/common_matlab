function data = extract_flight_data(mat_filepath, target_phase)
    ...
    data.cbf.A_xy_qp_list        = zeros(2, N); % QP制約係数 (1行目: ロール, 2行目: ピッチ)
    data.cbf.b_xy_qp_list        = zeros(1, N); % 入力u_23に許容される上限制約
    ...
    data.cbf.log_h               = zeros(4, N); % 安全余裕度 (1:位置, 2:速度, 3:加速度, 4:加加速度)
    ...
    for i = 1:N
        ...
        data.cbf.A_xy_qp_list(:, i)         = con_res.A_xy_qp_list(1:2);       % 1: ロールに対する制約係数, 2: ピッチに対する制約係数
        data.cbf.b_xy_qp_list(i)            = con_res.b_xy_qp_list(1);        % 入力u_23に許容される上限制約
        ...
        data.cbf.log_h(:, i)                = [con_res.log_h1(1); ...         % 位置の安全余裕度
                                               ...
                                               con_res.log_h3(1); ...         % 加速度の安全余裕度
                                               con_res.log_h4(1)];            % か加速度の安全余裕度
