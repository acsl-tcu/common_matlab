classdef DRAW_COOPERATIVE_DRONES
    ...
    methods
        function obj = DRAW_COOPERATIVE_DRONES(logger,varargin)
            ...
            if isfield(param,'load_index')
                obj.load_index = param.load_index;
            else
                obj.load_index = max(obj.target) + 1;
            end
            data = logger.data(obj.load_index,"p","e");
            tM = data;%max(data,1);
            tm = data;%min(data,1);
            ...
            p0 = data(1,:);
            q = obj.data_format(logger,obj.load_index,1,"plant.result.state.Q","p");
            [~,Q0] = obj.gen_Q(1,q(1,:));
            qi = obj.data_format(logger,obj.load_index,obj.target,"plant.result.state.qi","p");
        ...
        function obj=gen_frame(obj,varargin)
            ...
            for n = param.target
                for i = 4:-1:1
                    h(n,i) = surface(obj.ax,xr+rp(i,1),yr+rp(i,2),zr+rp(i,3),'FaceColor',c(i)); % rotor 描画
                    T(n,i) = quiver3(obj.ax,rp(i,1),rp(i,2),rp(i,3),0,0,1,'FaceColor',c(i)); % 推力ベクトル描画
                    tt(n,i) = hgtransform('Parent',obj.ax); set(T(n,i),'Parent',tt(n,i)); % 推力を慣性座標と紐づけ
                end
                ...
                h(n,6) = surface(obj.ax,xb,yb,zb,'FaceColor',c(5)); % ボディ楕円体

                t(n) = hgtransform('Parent',obj.ax); set(h(n,:),'Parent',t(n)); % ドローンを慣性座標と紐づけ
            end
            obj.frame = t;
            obj.thrust = tt;
        end
    function obj=gen_load(obj,varargin)
      ...
      if isfield(param,'self') && isprop(param.self, 'parameter') && ...
          isprop(param.self.parameter, 'pUp') && ~isempty(param.self.parameter.pUp) && ...
          isprop(param.self.parameter, 'pDown') && ~isempty(param.self.parameter.pDown)
        pUp = param.self.parameter.pUp;
        pDown = param.self.parameter.pDown;
        % Use polyshape+triangulation so NaN-separated holes render correctly.
        psUp = polyshape(pUp(1,:), pUp(2,:));
        psDown = polyshape(pDown(1,:), pDown(2,:));
        TRu = triangulation(psUp);
        TRd = triangulation(psDown);
        zUp = pUp(3, find(~isnan(pUp(3,:)), 1, 'first'));
        zDown = pDown(3, find(~isnan(pDown(3,:)), 1, 'first'));
        h(1) = patch(obj.ax, ...
            "Faces", TRu.ConnectivityList, ...
            "Vertices", [TRu.Points, zUp * ones(size(TRu.Points, 1), 1)], ...
            "FaceColor", "cyan", "FaceAlpha", 0.5, "EdgeColor", "none");
        h(2) = patch(obj.ax, ...
            "Faces", TRd.ConnectivityList, ...
            "Vertices", [TRd.Points, zDown * ones(size(TRd.Points, 1), 1)], ...
            "FaceColor", "cyan", "FaceAlpha", 0.5, "EdgeColor", "none");
        x = [pDown(1,:), pDown(1,1); pUp(1,:), pUp(1,1)];
        y = [pDown(2,:), pDown(2,1); pUp(2,:), pUp(2,1)];
        z = [pDown(3,:), pDown(3,1); pUp(3,:), pUp(3,1)];
        h(3) = surf(obj.ax, x, y, z, "FaceColor", "cyan", "FaceAlpha", 0.5);
        if isprop(param.self.parameter, 'G') && ~isempty(param.self.parameter.G)
          G = param.self.parameter.G;
          h(4) = plot3(obj.ax, G(1), G(2), G(3), "Marker", "*", "MarkerSize", 10, "Color", "red");
        end
      else
        [x,y,z] = cylinder(obj.ax,1,obj.N);
      ...
      ttt = hgtransform('Parent',obj.ax); set(h,'Parent',ttt); % 推力を慣性座標と紐づけ
      obj.load = ttt;
    end
        ...
        function animation(obj,logger,varargin)
            ...
            p = obj.data_format(logger,obj.load_index,1,"p","p");
            q = obj.data_format(logger,obj.load_index,1,"plant.result.state.Q","p");
            [Q,Q0] = obj.gen_Q(1,q);
            u = obj.data_format(logger,obj.target,obj.target,"input","");% N x 4m
            r = obj.data_format(logger,obj.load_index,1,"p","r");
            qi = obj.data_format(logger,obj.load_index,obj.target,"plant.result.state.qi","p");
