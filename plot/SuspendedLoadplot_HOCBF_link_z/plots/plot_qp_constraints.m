...
% 制約係数 A (1行目: Roll, 2行目: Pitch)
A_throttle  = data.cbf.A_z_qp_list(1, t_pos_idx);

% 許容上限 b
b_qp = data.cbf.b_z_qp_list(t_pos_idx);

% 各入力 [Roll; Pitch]
u_nom_throttle  = data.cbf.tmp(1, t_pos_idx);     % 補正前
u_fix_throttle  = data.cbf.tmp_fix(1, t_pos_idx); % 補正後

% 左辺 A * u の計算
A_u_nom = A_throttle .* u_nom_throttle; % 公称入力時の A*u
A_u_fix = A_throttle .* u_fix_throttle; % CBF補正後の A*u
