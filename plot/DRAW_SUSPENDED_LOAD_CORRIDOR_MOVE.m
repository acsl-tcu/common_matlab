...
classdef DRAW_SUSPENDED_LOAD_CORRIDOR_MOVE < DRAW_SUSPENDED_LOAD
    ...
    methods
        function obj = DRAW_SUSPENDED_LOAD_CORRIDOR_MOVE(logger, varargin)
            ...
            elseif isfield(param, 'self') && isprop(param.self, 'reference')
                try
                    ...
                    if iscell(func_list)
                        for i = 1:length(func_list)
                            if contains(class(func_list{i}), 'REPLANNING') || ...
                               contains(class(func_list{i}), 'BSPLINE') || ...
                               contains(class(func_list{i}), 'MELLINGER') || ...
                               contains(class(func_list{i}), 'SWING')
                                obj.replanner_ref = func_list{i};
                                break;
                            end
        ...
        function draw(obj, varargin)
            ...
            % 計画軌道オーバーレイ描画
            % -------------------------------------------------------------
            if ~isempty(obj.replanner_ref) && isprop(obj.replanner_ref, 'replan_active') && obj.replanner_ref.replan_active
                if obj.cached_traj_t_start ~= obj.replanner_ref.t_start
                    delete(findobj(obj.ax, 'Tag', 'TAG_CORRIDOR_PLANNED_TRAJ'));
                    obj.draw_planned_trajectories();
                    obj.cached_traj_t_start = obj.replanner_ref.t_start;
    ...
    methods (Access = private)
        ...
        function draw_planned_trajectories(obj)
            ...
            for i = 1:N_pts
                ...
                % 機体目標軌道の抽出
                if length(xd_opt) >= 23 && any(xd_opt(21:23) ~= 0)
                    pQ_traj(:, i) = xd_opt(21:23);
                else
                    aL = xd_opt(9:11);
                    acc_tot = aL + g_vec;
                    if norm(acc_tot) > 1e-3
                        thrust_dir = acc_tot / norm(acc_tot);
                    else
                        thrust_dir = [0; 0; 1];
                    end
                    pQ_traj(:, i) = pL_traj(:, i) + L_val * thrust_dir;
                end
            end
