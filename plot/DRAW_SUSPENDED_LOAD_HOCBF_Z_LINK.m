% classdef DRAW_SUSPENDED_LOAD_HOCBF_Z_LINK
...
% 
%     methods
%         function obj = DRAW_SUSPENDED_LOAD_HOCBF_Z(logger, varargin)
...
%             end
% 
%             %% 🛡️ 【新規追加】単機牽引システム側ののグラフィックス初期化　後々実際の値を持ってこれるようにしよう
%             if isfield(param, "rl")
%                 r_sphere = param.rl;
%             else
%                 % フォールバックは一切行わず、設定漏れを検知して強制停止
%                 error("DRAW_SUSPENDED_LOAD_AVOIDANCE:MissingRlParameter", ...
%                     "研究用設定の不整合を防ぐため、引数 'rl'（荷物の物理半径）の明示的な指定が必須です。");
%             end
% 
%             [px, py, pz] = sphere(16);
% 
%             % 💡 実際の牽引物データ(pL)の最初のフレームから、直接座標を取得して配置
%             obj.protection_spheres(1) = surf(obj.ax, ...
%                 px * r_sphere + pL(1,1), ...
%                 py * r_sphere + pL(1,2), ...
%                 pz * r_sphere + pL(1,3), ...
...
%                 current_XData = get(obj.protection_spheres(1), 'XData');
%                 r_sphere = (max(current_XData(:)) - min(current_XData(:))) / 2;
% 
%                 [px, py, pz] = sphere(16);
% 
%                 % 制約の対象である最新の牽引物位置 pL(1,:) に直接球を配置
%                 set(obj.protection_spheres(1), ...
%                     'XData', px * r_sphere + pL(1,1), ...
%                     'YData', py * r_sphere + pL(1,2), ...
%                     'ZData', pz * r_sphere + pL(1,3));
%             end
%             drawnow
%         end
% 
...
classdef DRAW_SUSPENDED_LOAD_HOCBF_Z_LINK
    % 障害物回避対応版：ドローン＋吊り荷の3次元アニメーション表示クラス
    % 論文準拠の「ロープ（リンク）中心を包む保護球」と、障害物の「本体・安全マージン・CBF領域」を半透明で完全可視化します。
    properties
        frame
        thrust
        ax
        ...
        % --- 回避表示用のグラフィックス・ハンドル群 ---
        obs_handles = [];        % 障害物の表示オブジェクト
        protection_spheres = []; % ロープ（リンク）中心を包む保護球オブジェクト
    end
    methods
        function obj = DRAW_SUSPENDED_LOAD_HOCBF_Z_LINK(logger, varargin)
            param = struct(varargin{:});
            if ~isfield(param, "target")
                param.target = 1;
            end
            ...
            %% 🛡️ 保護球グラフィックス初期化（ロープ・リンク中心 p_mid に配置）
            if isfield(param, "rl")
                r_sphere = param.rl;
            else
                error("DRAW_SUSPENDED_LOAD_AVOIDANCE:MissingRlParameter", ...
            ...
            % 🌟 初期フレームにおけるロープ（リンク）中心座標 p_mid の算出
            p0 = p(1,:);
            if isempty(pL)
                pL0 = p0 + [0 0 -obj.load_size(3)];
            else
            ...
            % 保護球を中心座標 p_mid0 に初期配置
            obj.protection_spheres(1) = surf(obj.ax, ...
                px * r_sphere + p_mid0(1), ...
                py * r_sphere + p_mid0(2), ...
                pz * r_sphere + p_mid0(3), ...
                'FaceColor', [0.2 0.8 1.0], 'EdgeColor', 'none', 'FaceAlpha', 0.35);
            view(ax, 3)
            grid(ax, 'on')
            daspect(ax, [1 1 1]);
        ...
        function draw(obj, target, p, q, u, pL)
            ...
            if ~isempty(pL)
                ...
                %% 🔄 保護球のリアルタイム位置をロープ（リンク）中心 p_mid に同期
                p_drone = p(1,:,1);
                p_load  = pL(1,:);
                p_mid   = 0.5 * (p_drone + p_load); % 🌟 ロープ（リンク）中心点の動的算出
                ...
                % 制約の対象である最新のロープ（リンク）中心点 p_mid に直接球を配置
                set(obj.protection_spheres(1), ...
                    'XData', px * r_sphere + p_mid(1), ...
                    'YData', py * r_sphere + p_mid(2), ...
                    'ZData', pz * r_sphere + p_mid(3));
            end
            drawnow
        end
...
% classdef DRAW_SUSPENDED_LOAD_HOCBF_Z_LINK
%     % 障害物回避対応版：ドローン＋吊り荷の3次元アニメーション表示クラス
%     % 論文・マルチセグメント球体モデル準拠：システム（荷物・ケーブル・機体）を被覆する複数の保護球と、
%     % 障害物の「本体・安全マージン・CBF領域」を半透明で可視化します。
%     properties
%         frame
%         thrust
%         ax
...
%         % --- 回避表示用のグラフィックス・ハンドル群 ---
%         obs_handles = [];        % 障害物の表示オブジェクト
%         protection_spheres = []; % 複数被覆球の表示オブジェクト群 (配列)
% 
%         % --- マルチセグメント球体用パラメータ ---
%         lambda_list = [0.0, 0.5, 1.0]; % デフォルト位置比率 (荷物〜機体)
%         rl_list     = [0.2, 0.15, 0.35];% デフォルト各球体半径 [m]
%     end
%     methods
%         function obj = DRAW_SUSPENDED_LOAD_HOCBF_Z_LINK(logger, varargin)
%             param = struct(varargin{:});
...
%             end
% 
%             % 🌟 マルチセグメント球体パラメータの引数取得（制御クラスと同期）
%             if isfield(param, "lambda_list")
%                 obj.lambda_list = param.lambda_list;
%             end
%             if isfield(param, "rl_list")
%                 obj.rl_list = param.rl_list;
%             elseif isfield(param, "rl") % 単一引数指定への後方互換
%                 obj.rl_list = repmat(param.rl, size(obj.lambda_list));
%             end
% 
%             p = obj.data_format(logger, param.target, "p", "p");
%             pL = [];
...
%             end
% 
%             %% 🛡️ マルチセグメント保護球グラフィックス初期化（全球体の生成）
%             [px, py, pz] = sphere(16);
%             p0 = p(1,:);
%             if isempty(pL)
%                 pL0 = p0 + [0 0 -obj.load_size(3)];
...
%             end
% 
%             num_spheres = length(obj.lambda_list);
%             obj.protection_spheres = gobjects(1, num_spheres);
% 
%             for j = 1:num_spheres
%                 lambda_j = obj.lambda_list(j);
%                 rl_j     = obj.rl_list(j);
% 
%                 % 位置 x_c^j = x_L + lambda_j * (x_Q - x_L) の初期配置
%                 x_c_j = pL0 + lambda_j * (p0 - pL0);
% 
%                 obj.protection_spheres(j) = surf(obj.ax, ...
%                     px * rl_j + x_c_j(1), ...
%                     py * rl_j + x_c_j(2), ...
%                     pz * rl_j + x_c_j(3), ...
%                     'FaceColor', [0.2 0.8 1.0], 'EdgeColor', 'none', 'FaceAlpha', 0.25);
%             end
% 
%             view(ax, 3)
%             grid(ax, 'on')
...
% 
%                     % リアルタイム座標 x_c^j の更新
%                     x_c_j = p_load + lambda_j * (p_drone - p_load);
% 
%                     set(obj.protection_spheres(j), ...
%                         'XData', px * rl_j + x_c_j(1), ...
%                         'YData', py * rl_j + x_c_j(2), ...
%                         'ZData', pz * rl_j + x_c_j(3));
%                 end
%             end
%             drawnow
%         end
