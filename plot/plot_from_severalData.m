...
folder = uigetdir(pwd, "PDFの保存先を選択");
% fprintf(filename);
...
%  実験ファイルごとの処理
% =============================
for exp_idx = 1:numel(filenames)

    filename = filenames{exp_idx};

    fprintf('\n=============================\n');
    fprintf('実験 %d / %d\n', exp_idx, numel(filenames));
    fprintf('%s\n', filename);
    fprintf('=============================\n');
    ...
    rmse_text = disp_rmse(logger, settings.phase);
    ...
    rmse_file = fullfile( ...
        settings.savefolder, ...
        sprintf('%s_RMSE.txt', settings.savename));

    fid = fopen(rmse_file, 'w');
...
att = "er";     %erでいいやって時は，これだけcomentOutして，それ以外in
...
function rmse_text = disp_rmse(logger, phase)
...
for i = 1:length(target)
    ...
    % メモ保存用
    rmse_text = rmse_text + sprintf( ...
        '%s RMSE: %s\nsum = %.6f\n', ...
        target(i), ...
        strtrim(sprintf('%.6f ', RMSE)), ...
        sum(RMSE));
