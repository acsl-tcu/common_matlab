...
%       ConnectToNatNet - Updated version of connect
...
classdef natnet < handle
	...
	methods ( Access = public  )
		...
		function [validConnection] = ConnectToNatNet(obj, clientIP, hostIP, ConnectionType)
			validConnection = 0;
			obj.HostIP = hostIP;
			...
			if ( obj.IsConnected == 1 )
				...
				for i = 1 : modelDescription.TrackingModelCount
					...
					elseif (descriptor.type == 4)
						...
						modelDescription.Devices( modelDescription.DeviceCount).Name = char(descriptor.Name);
						modelDescription.Devices( modelDescription.DeviceCount).ID  = descriptor.ID;
						modelDescription.Devices( modelDescription.DeviceCount).Serial = descriptor.Serial;
						modelDescription.Devices( modelDescription.DeviceCount).Channel_Data_Type = descriptor.ChannelDataType;
						modelDescription.Devices( modelDescription.DeviceCount).Device_Type = descriptor.DeviceType;
						modelDescription.Devices( modelDescription.DeviceCount).ChannelCount = descriptor.ChannelCount;
					...
					elseif (descriptor.type == -1 || descriptor.type == 5)
						...
						modelDescription.Cameras(modelDescription.CameraCount).Name = char(descriptor.Name);
						%TBD Position
						modelDescription.Cameras(modelDescription.CameraCount).PosX = descriptor.x;
						modelDescription.Cameras(modelDescription.CameraCount).PosY = descriptor.y;
						modelDescription.Cameras(modelDescription.CameraCount).PosZ = descriptor.z;
						modelDescription.Cameras(modelDescription.CameraCount).OrientX = descriptor.qx;
						modelDescription.Cameras(modelDescription.CameraCount).OrientY = descriptor.qy;
						modelDescription.Cameras(modelDescription.CameraCount).OrientZ = descriptor.qz;
