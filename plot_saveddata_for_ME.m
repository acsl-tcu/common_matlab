...
%% 実験情報入力
fprintf('\n=============================\n');
fprintf('実験情報を入力してください。\n');
fprintf('各項目は N と入力すると次の項目へ進みます。\n');
fprintf('=============================\n');

rmse = inputMultiLine("RMSE");
program = inputMultiLine("使用プログラム");
runtime = inputMultiLine("実行時間");
inputflag = inputMultiLine("入力有無");
memo = inputMultiLine("自由メモ");

%% メモ保存
memo_file = fullfile( ...
    settings.savefolder, ...
    sprintf('%s_memo.txt', settings.savename));

fid = fopen(memo_file, 'w');
...
%% 複数行入力用の関数
function text = inputMultiLine(title)
    fprintf('\n--- %s ---\n', title);
