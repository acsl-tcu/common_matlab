function results_edmd = train_edmd_residual_from_logs(opts)
% HLC_EDMD_MEC.m 用の単一ログ群学習関数
% loggerデータから公称EDMDモデルと残差EDMDモデルを学習する
% TRAIN_EDMD_RESIDUAL_FROM_LOGS
% Shared trainer for residual EDMD models from logger files.
    ...
    opts = set_default(opts, 'lambda_nom', 1e-6);
    opts = set_default(opts, 'lambda_err', 1e-3);
    opts = set_default(opts, 'pos_err_thresh', 1.5);
    opts = set_default(opts, 'vel_thresh', 5.0);
    opts = set_default(opts, 'ang_thresh', pi / 3);
    opts = set_default(opts, 'obs_norm_thresh', 200.0);
    opts = set_default(opts, 'res_norm_thresh', 50.0);
    ...
    opts = set_default(opts, 'mad_scale', 6.0);
    ...
    for fi = 1:numel(files)
        ...
        for k = 1:(N_steps - 1)
            ...
            % Keep the lifted input context aligned with the control action at k.
            % Online compensation evaluates z(x_k, u_k) and predicts x_{k+1}
            % under the same input, so the training target must use u_k as well.
            x16_t = [x12_t; u_nom_t];
    ...
    mask_du_abs = du_norm <= opts.du_abs_thresh;
    mask_du_ratio = du_ratio <= opts.du_ratio_thresh;
    mask_jump_hard = z_jump <= opts.jump_norm_hard_thresh;
    mask_du_mad = robust_upper_mask(du_norm, opts.mad_scale);
    mask_jump_mad = robust_upper_mask(z_jump, opts.mad_scale);
    ...
    % 公称EDMDモデル A_nom, B_nom を学習する
    fprintf('\\n=== Train nominal EDMD ===\\n');
    [A_nom, B_nom] = solve_ridge(Z, U_NOM, ZN, opts.lambda_nom, opts.use_gpu);
    ZN_nom = A_nom * Z + B_nom * U_NOM;
    rmse_nom = sqrt(mean((ZN - ZN_nom).^2, 'all'));
    fprintf('nominal RMSE: %.6e\n', rmse_nom);

    % 公称モデルで説明できない残差モデル A_err, B_err を学習する
    fprintf('\\n=== Train residual EDMD ===\\n');
    ZBAR = ZN - ZN_nom;
...
% 参照姿勢角を取得する
    ...
    elseif isprop(st, 'xd') || (isstruct(st) && isfield(st, 'xd'))
        xd = double(st.xd(:));
        if numel(xd) < 6
            error('reference.state.xd is too short to extract q.');
        end
        q = xd(4:6);
    else
...
function mask = robust_upper_mask(x, mad_scale)
    ...
    ub = med_x + mad_scale * robust_sigma;
    mask = x <= ub;
end
