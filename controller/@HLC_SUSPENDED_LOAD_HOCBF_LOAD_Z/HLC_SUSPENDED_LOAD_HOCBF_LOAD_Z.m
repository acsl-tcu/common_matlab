...
classdef HLC_SUSPENDED_LOAD_HOCBF_LOAD_Z < handle
% 手法2: 中村らの研究ベース（補正量 Delta_u1 の直接算出）
properties
    self
    result
    param
...
methods
    ...
    function result = do(obj, varargin)
        ...
        max_violation = -inf;
        delta_u1_selected = 0.0;
        
        % 全障害物についてループを回し、最も危険な違反を叩き出している障害物の補正量を計算する
        for i = 1:num_obs
            xo = obs_env(i).p_obs(1);
            yo = obs_env(i).p_obs(2);
            zo = obs_env(i).p_obs(3);
            ...
            % ノミナル入力を用いた現在値(I)と目標安全境界(J)の評価
            I_val = A_i * u1_nominal;
            J_val = B_i;
            
            % 危険度（制約違反量）の算出
            violation = J_val - I_val;
            
            if violation > 0 && A_i ~= 0
                % 複数障害物のうち、最も危険な（違反が最大の）障害物を優先選択
                if violation > max_violation
                    max_violation = violation;
                    % 導出式: Delta_u1 = (J - I) / A_i
                    delta_u1_selected = (J_val - I_val) / A_i;
                end
            end
        end
        obj.result.t_loop = toc(tic_loop); 
        ...
        % 3. 補正量の適用
        tic_qp = tic;
        u1_safe = u1_nominal + delta_u1_selected;
        ...
        %% =========================================================================
        % 安全のため入力値に制限を付ける．
        obj.result.input = [max(0, min(20, tmp(1))); ... % 安全化された u1 (推力) + 後段物理クランプ
                            max(-1, min(1, tmp(2))); ...  % u2 (roll)
                            max(-1, min(1, tmp(3))); ...  % u3 (pitch)
                            max(-1, min(1, tmp(4)))];    % u4 (yaw)
