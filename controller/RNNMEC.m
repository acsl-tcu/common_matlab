classdef RNNMEC < handle
    ...
    properties
        ...
        agent
        RNN_model_filename  % 読み込みたいONNXモデルのファイル名
    ...
    methods
        function obj = RNNMEC(self, RNN_model_filename)
            % インスタンス
            obj.self = self;
            obj.param = self.parameter.get(obj.parameter_name);

            %-%-%-% DNN model import & define %-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%
            obj.RNN_model_filename = RNN_model_filename;
            ...
            RNN_model = importNetworkFromONNX("\DNN_MODEL\"+obj.RNN_model_filename,... % ONNXファイルインポート
                                                "InputDataFormats", "TBC", ... % 入力層定義 "BCT" -> [時間窓の幅，バッチサイズ，特徴量]
                                                "OutputDataFormats", "BC");   % 出力層定義 "BC" -> [バッチサイズ, 特徴量]の意味
            if contains(RNN_model_filename, '21'),  dim = 21;
            else,                                   dim = 24; end
            time_step_length = 10;
            tmp = randn(dim, time_step_length, 'single');
            dummyInput = dlarray(reshape(tmp, [dim,1,time_step_length]), 'CBT'); % 初期化のためのdummy入力
            obj.RNNMEC_model = initialize(RNN_model, dummyInput); % モデルの初期化
            %-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%
        ...
        function result = do(obj, varargin)
            ...
            % DNN関係　閾値での制限
            obj.result.delta_input = -1*double(predict(obj.RNNMEC_model, data'))'; % predict関数での推論
