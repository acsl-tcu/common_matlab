classdef NNMEC < handle
    ...
    methods
        function obj = NNMEC(self, NN_model_filename)
            ...
            if ~exist("controller\NN_MODEL", "dir")
                mkdir("controller\NN_MODEL")
            elseif isempty(dir("controller\NN_MODEL\*.onnx"))
                error("ACSL: Do not exist <onnx> file in controller\NN_MODEL. ")
            end
            NN_model = importNetworkFromONNX("\NN_MODEL\"+obj.NN_model_filename,... % ONNXファイルインポート
        ...
        function result = do(obj, varargin)
            ...
            obj.result.delta_input = -1*double(predict(obj.NNMEC_model, data'))'; % predict関数での推論
        ...
        function Delta_input = infer_model(obj, plant_state, nominal_state)
            ...
            for row = 1:size_plant(2)
                ...
                Delta_input(:,row) = -1*double(predict(obj.NNMEC_model, data'))'; % predict関数での推論
            end
        end
    end
end
