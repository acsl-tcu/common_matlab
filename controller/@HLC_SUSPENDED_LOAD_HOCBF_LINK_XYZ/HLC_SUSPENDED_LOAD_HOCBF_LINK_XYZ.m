...
classdef HLC_SUSPENDED_LOAD_HOCBF_LINK_XYZ < handle
  % 単機 HLC_CBF_ActuatorCBF と完全同一設計の牽引紐中点コントローラ
  % 決定変数: mu = [ddT; tau_x; tau_y; tau_z] \in R^4
  properties
    self
    result
    param
  ...
  methods
    ...
    function result = do(obj, varargin)
      ...
      % =========================================================================
      % 0. 牽引物質量 (mL) 推定値の取得 (未推定なら停止)
      % =========================================================================
      loadmass_val = [];
      if isfield(model, 'state')
        if isprop(model.state, 'mL') && ~isempty(model.state.mL)
          loadmass_val = model.state.mL;
        elseif isfield(model.state, 'mL') && ~isempty(model.state.mL)
          loadmass_val = model.state.mL;
        end
      end
      if isempty(loadmass_val) && isfield(model, 'mL') && ~isempty(model.mL)
        loadmass_val = model.mL;
      end

      if isempty(loadmass_val) || isnan(loadmass_val)
        error('HLC_SUSPENDED_LOAD_ActuatorCBF:NoLoadMass', ...
              '荷物質量 (mL) の推定値が存在しません。安全のため停止します。');
      end
      ...
      % 物理パラメータ P
      drone_P = obj.self.parameter.get(["mass", "jx", "jy", "jz", "gravity", "cableL"]);
      m_drone = drone_P(1);
      jx_val  = drone_P(2);
      jy_val  = drone_P(3);
      jz_val  = drone_P(4);
      ...
      % 階層型線形化用 P: [m, jx, jy, jz, g, mL, L, 0, 0, 0]
      P = [m_drone, jx_val, jy_val, jz_val, g_val, loadmass_val, L_cable, 0, 0, 0];

      % 状態ベクトル x: [q; w; pL; vL; pT; wL]
      q_compact = model.state.getq('compact');
      x = [q_compact; model.state.w; pL; model.state.vL; pT; model.state.wL];
      ...
      deltaYaw = sign(cross(yawdUnit, yawUnit)) * acos(yawdUnit' * yawUnit);
      xd(4)    = -deltaYaw(3) + yaw;
      xd = [xd; zeros(max(0, 28 - size(xd, 1)), 1)];
      ...
      Param = obj.param;
      F1 = Param.F1; F2 = Param.F2; F3 = Param.F3; F4 = Param.F4;
      vf = obj.Vfd_SuspendedLoadxyDst(dt, x, xd', F1);
      vs = obj.Vs_SuspendedLoadxyDst(x, xd', vf, P, F2, F3, F4);
      uf = obj.Uf_SuspendedLoadxyDst(x, xd', vf, P);
      beta2 = obj.Beta2_SuspendedLoadxyDst(x, xd', vf, P);
      vs_alpha2 = obj.V2_alpha2_SuspendedLoadxyDst(x, xd', vf, vs', P);
      ...
      % =========================================================================
      % 3. 障害物 CBF (牽引紐中点 相対次数4)
      % =========================================================================
      p_mid = pL - 0.5 * L_cable * pT;
      rl_sys = 1.0; % 中点マージン球半径

      % Sphere_SuspendedLoad_TetherMid_CBF 用の状態・パラメータ
      z_state = [pL; model.state.vL; pT; model.state.wL; q_compact; model.state.w];
      T_state = [obj.T_val; obj.dT_val];
      drone_params = [m_drone; loadmass_val; L_cable; jx_val; jy_val; jz_val; g_val];
      cbf_gains = [80.0; 10.0; 3.0; 1.5]; % [k0; k1; k2; k3]

      obs_list = ENVIRONMENT_OBSTACLE_HOCBF_LINK_XY();
      obj.result.min_clearance = Inf;

      for i = 1:length(obs_list)
        obs_p = obs_list(i).p_obs(:);
        obs_r = obs_list(i).r_obs_margin;
        obs_params = [obs_p; obs_r + rl_sys];

        % 幾何クリアランス更新
        dist_mid = norm(p_mid - obs_p) - (obs_r + rl_sys);
        if dist_mid < obj.result.min_clearance
          obj.result.min_clearance = dist_mid;
        end

        % 水平3.0m以内のみ評価
        if norm(p_mid(1:2) - obs_p(1:2)) > (obs_r + rl_sys + 3.0)
          continue;
        end

        [A_cbf, b_cbf, h_val] = Sphere_SuspendedLoad_TetherMid_CBF(...
            z_state, T_state, drone_params, obs_params, cbf_gains);

        if ~any(isnan(A_cbf)) && ~any(isnan(b_cbf))
          A_cbf_all = [A_cbf_all; double(A_cbf)];
          b_cbf_all = [b_cbf_all; double(b_cbf)];
        end
      end

      % =========================================================================
      % 4. 推力制限 Actuator CBF (単機と完全同一)
      % =========================================================================
      T_min = 1.0;
      T_max = 20.0;
      ...
      % =========================================================================
      % 6. 推力積分更新 ＆ 最終クリッピング (単機と完全同一)
      % =========================================================================
      obj.dT_val = obj.dT_val + mu_safe(1) * dt;
      obj.T_val  = obj.T_val  + obj.dT_val * dt;
      obj.T_val  = max(0.0, min(20.0, obj.T_val));

      obj.result.input = [obj.T_val; ...
                          max(-1.0, min(1.0, mu_safe(2))); ...
                          max(-1.0, min(1.0, mu_safe(3))); ...
                          max(-1.0, min(1.0, mu_safe(4)))];
      obj.result.p_mid = p_mid;
      obj.result.rl    = rl_sys;
      result = obj.result;
    end
