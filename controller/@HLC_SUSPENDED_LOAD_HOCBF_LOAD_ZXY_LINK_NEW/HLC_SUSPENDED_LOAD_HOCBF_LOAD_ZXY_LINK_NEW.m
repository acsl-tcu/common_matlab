...
classdef HLC_SUSPENDED_LOAD_HOCBF_LOAD_ZXY_LINK_NEW < handle
    ...
    methods
        ...
        function result = do(obj, varargin)
            ...
            % --- 牽引物質量 (loadmass) 推定値の取得チェック ---
            loadmass_val = [];
            if isprop(model.state, 'loadmass') && ~isempty(model.state.loadmass)
                loadmass_val = model.state.loadmass;
            elseif isprop(model.state, 'param') && isfield(model.state.param, 'loadmass') && ~isempty(model.state.param.loadmass)
                loadmass_val = model.state.param.loadmass;
            elseif isfield(model, 'loadmass') && ~isempty(model.loadmass)
                loadmass_val = model.loadmass;
            end
            
            % 推定値が存在しない場合はフォールバック値を使わずに停止
            if isempty(loadmass_val) || isnan(loadmass_val)
                error('HLC_SUSPENDED_LOAD:EstimatedLoadMassNotFound', ...
                      '牽引物質量の推定値が存在しません。安全のため制御器の実行を停止しました。');
            end
            ...
            % 機体パラメータの取得と牽引物質量推定値の結合
            drone_P = obj.self.parameter.get(["mass", "jx", "jy", "jz", "gravity", "cableL"]);
            % [mass, jx, jy, jz, gravity, loadmass, cableL, 0, 0]
            P = [drone_P(1:5), loadmass_val, drone_P(6), 0, 0];
            ...
            %% =========================================================================
            %% 【ステップ 2】 実入力 u1 保持型 CBF_Constraints_HOCBF_xylink による xy 方向 QP 安全補正
            %% =========================================================================
            u23_nominal = [tmp(2); tmp(3)];
            u1_val      = tmp(1);
            u4_val      = tmp(4);
            gamma_params_xy = [2; 8; 16; 32];
