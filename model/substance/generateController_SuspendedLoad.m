...
% disp('========================================================================');
%% =========================================================================
%% 論文完全準拠：ドローン＆荷物 両軸2次（相対次数2）CBF自動導出セクション
%% =========================================================================
clc
disp('========================================================================');
disp(' 論文の幾何学定義に基づき、両軸を「相対次数2」に統一したCBFの生成を開始します');
disp('========================================================================');

% 1. 公称システムのダイナミクスの抽出（外乱は0として公称閉ループを構成）
f_nominal = subs(f, [dstx, dsty], [0, 0]); 
g_nominal = g; 

% 2. 最上流の仮想操作量（加速度次元）の定義
syms v1_cmd u2_cmd u3_cmd real
u_top_layer = [v1_cmd; u2_cmd; u3_cmd];

% パラメータの定義（k_cbf3〜k_cbf5は2次化に伴い未使用となりますが互換性のために保持）
...
%% -----------------------------------------------------------------
%% 2. 荷物（ペイロード）のCBF (🌟5次から「論文準拠の2次」へ大胆に書き換え)
%% -----------------------------------------------------------------
disp('▶️ [Load Layer] 荷物側の距離関数を「2次(相対次数2)」として再構成中...');
p_load = [pl(1); pl(2); pl(3)]; 

% 荷物側の基本距離関数 h0
h0_load_sym = 0.5 * ((p_load(1) - ox)^2 + (p_load(2) - oy)^2 + (p_load(3) - oz)^2 - (ro + r_load)^2);

% 🌟 5次の深いバックステッピングを全廃し、ドローンと同じく2次（2階のLie微分）で最上流へ直結させます。
% 1段目の全微分
dot_h_load = LieD(h0_load_sym, FG_top_drone, x);
phi1_load  = dot_h_load + k_cbf1 * h0_load_sym; % 1つ目のゲイン(k_cbf1)を使用

% 2段目の全微分（荷物側もここで仮想加速度入力 [v1, u2, u3] と代数的に直結）
dot_phi1_load_total = LieD(phi1_load, FG_top_drone, x);

% 【確実な代数分離】jacobianによるAの抽出と、残差によるbの抽出
A_load_top_sym = jacobian(dot_phi1_load_total, u_top_layer);
b_pure_load    = simplify(dot_phi1_load_total - A_load_top_sym * u_top_layer);
b_load_top_sym = b_pure_load + k_cbf2 * phi1_load; % 2つ目のゲイン(k_cbf2)を使用
...
%% -----------------------------------------------------------------
%% 4. Mファイル関数としてのエクスポート（互換性完全維持）
%% -----------------------------------------------------------------
disp('💾 新・最上流同期型2次CBF関数の書き出し中...');

% 仮想入力のゴミ（subs残り）を完全に消滅させて純粋な加速度の次元の係数にするクリーンアップ
A_load_top_clean  = subs(A_load_top_sym,  [v1_cmd, u2_cmd, u3_cmd], [0, 0, 0]);
b_load_top_clean  = subs(b_load_top_sym,  [v1_cmd, u2_cmd, u3_cmd], [0, 0, 0]);
A_drone_top_clean = subs(A_drone_top_sym, [v1_cmd, u2_cmd, u3_cmd], [0, 0, 0]);
b_drone_top_clean = subs(b_drone_top_sym, [v1_cmd, u2_cmd, u3_cmd], [0, 0, 0]);

% 🌟 既存のコントローラ（do関数）の変更を不要にするため、
% 使用しなくなったk_cbf3〜k_cbf5の位置には元のシンボリック変数をそのままダミーとしてパッキングします。
cbfParam_top_load  = [ox; oy; oz; ro; r_load;  k_cbf1; k_cbf2; k_cbf3; k_cbf4; k_cbf5; physicalParam(:)];
cbfParam_top_drone = [ox; oy; oz; ro; r_drone; d1; d2; physicalParam(:)];
XD_vars = cell2sym(XD);
...
disp('========================================================================');
disp('🎉 [2次化成功] 論文と等価な軽量・高安定CBF制約関数ファイルが再生成されました！');
disp('========================================================================');
%% =========================================================================
