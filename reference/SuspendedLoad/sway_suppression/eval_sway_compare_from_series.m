...
%   t, p, v, pL, vL
% Optional fields:
%   phase (numeric), cha (char/cellstr), xd or pref, etc.
...
% ★ flight crop が空なら即return（保険）
if isempty(A0.t) || isempty(A1.t)
    error('Flight crop resulted in empty series. Check opt.flight_cha and cha mapping.');
end

% ---------- 2) alignment (robust) ----------
% ★ detect_start_time は使わず、「flight開始=0」に固定
if opt.align
    A0.t = A0.t - A0.t(1);
    A1.t = A1.t - A1.t(1);
end
...
% flight の連続区間を1つだけ取りたい（最長区間を採用）
mask = keep_longest_true_segment(mask);

A = apply_mask(S, mask);

% ★ flight が0点なら落ちるのでフォールバック
if isempty(A.t)
...
% Make "start time" = 0 for both, based on detected event
% and crop [t0-pre, t0+post]
...
if mode == "ref_move"
    % detect when reference starts moving:
    % If A has xd or pref -> use it, else fallback to p movement
    if isfield(A,'xd') && ~isempty(A.xd)
        pref = A.xd(:,1:3);
    elseif isfield(A,'pref') && ~isempty(A.pref)
        pref = A.pref(:,1:3);
    else
        % fallback: use drone position (less ideal)
        pref = A.p(:,1:3);
    end
...
% additional "paper" scalars
out.max_rxy = max(rxy_n);
out.rxy_rms = rms(rxy_n);

% envelope of S(t) (optional, for plotting)
...
% x is theta; settle when |x| <= band and remains
k = [];
