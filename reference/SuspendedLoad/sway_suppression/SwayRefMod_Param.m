...
% 危険域での補正強化
p.gain_boost = 2.0;             % alpha=2.0 など  危険域のゲインスケジューリング倍率（※CBFではない）
...
p.kv_vref = 1.0; %0.6 %0.3~1.0 % apply_to_vref=trueにするなら小さく（例0.2）
p.dv_max  = 1.0; %0.6 %0.3~0.8
p.dv_lpf_tau= 0.05;  % 0.03〜0.08
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
p.c_max = 0.3;   % [m] 補正の最大値（まずは10cm）目標の「追従不能」を避ける安全上限（重要）0.15
p.softstart_sec = 0.10;  % ON後0.3秒はゆっくり  ON直後だけゆっくり入れる（位相遅れ対策）0.3
...
p.S_smooth_tau   = 0.08;  % [s] S平滑 0.1〜0.3 0.15
%-------------------------------------------------------------------------
