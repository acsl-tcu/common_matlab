classdef REPLANNING_TUBE_MPC_MINVO < handle
    % REPLANNING_TUBE_MPC_MINVO
    % 案1: Tube-based Robust MPC と MINVO 基底による厳密な前方不変多項式生成
    % Fu et al. (CCC 2024) に基づき、誤差系の RPI (Robust Positively Invariant set)
    % を用いて制約を縮退 (Tighten) させ、分離超平面と 13次多項式 QP で安全を保証する。
    ...
    properties
        ...
        % Tube-based MPC のための外乱・誤差パラメータ
        d_max = 1.0; % 推定最大外乱 (m/s^2)
        K_e   = 2.5; % 誤差系の収束ゲイン (位置フィードバックゲイン相当)
        ...
        function result = do(obj, varargin)
            ...
            if cha == 'f' && ~obj.replan_done && ~obj.replan_active
                ...
                if min_dist <= obj.trigger_dist
                    fprintf("\n=======================================================\n");
                    fprintf("[TUBE MPC MINVO] 障害物接近検知! (min_dist=%.2fm, t=%.3f s)\n", min_dist, time.t);
                    obj.t_start = time.t;
                    ...
                    % 巻き戻りバグ防止: 始端状態を公称軌道に拘束
                    p0_ref = xd_nominal(1:3);
                    v0_ref = xd_nominal(5:7);
                    
                    fprintf("[TUBE MPC MINVO] RPIマージンの計算と分離超平面QPの実行...\n");
                    obj.plan_tube_mpc_trajectory(xd_nominal, dir_nom, spd, total_dist, p0_ref, v0_ref);
            ...
            if obj.replan_active
                ...
                if tau <= obj.t_duration
                    ...
                    if ~obj.replan_done
                        fprintf("[TUBE MPC MINVO] 障害物クリア・C^6シームレス合流 (t=%.3f s)\n\n", time.t);
    ...
    methods (Access = private)
        function plan_tube_mpc_trajectory(obj, xd0, dir_nom, spd, total_dist, p0_ref, v0_ref)
            T = obj.t_duration;
            p_end = p0_ref + dir_nom * total_dist;
            
            % --- 1. RPI (Robust Positively Invariant set) の計算と制約の縮退 ---
            % 誤差系 dot(e) = -K_e * e + d に対して、リアプノフ解析から
            % 不変集合のバウンドは ||e|| <= d_max / K_e となる。
            Delta_tube = obj.d_max / obj.K_e;
            
            % 制約の縮退 (Constraint Tightening)
            % 名目系が避けるべき安全距離に、RPIのサイズ (Delta_tube) を上乗せする。
            R_tight = obj.obs_radius + obj.safe_margin + obj.L_cable * 0.5 + Delta_tube;
            
            fprintf("    - 推定外乱バウンド: %.2f, 収束ゲイン: %.2f\n", obj.d_max, obj.K_e);
            fprintf("    - RPI (Tube) マージン: %.3f m\n", Delta_tube);
            fprintf("    - 縮退安全半径 (Tightened): %.3f m\n", R_tight);
            
            % --- 2. 分離超平面 (Separating Hyperplane) の計算 ---
            ...
            a_h = -dir_normal;
            p_plane = obj.obs_center + a_h * R_tight;
            b_h = dot(a_h, p_plane);
            
            % --- 3. 13次多項式 QP の定式化 (Minimum Crackle / Snap) ---
            order = 13;
            n_coeffs = order + 1;
            num_vars = 3 * n_coeffs; 
            ...
            if exitflag ~= 1
                fprintf("    [WARNING] 再帰的実現可能性 (Recursive Feasibility) を満たせませんでした (exitflag=%d)。制約を緩和します。\n", exitflag);
                bineq = bineq + 0.5;
                [C_opt, ~, exitflag] = quadprog(H, f, Aineq, bineq, Aeq, beq, [], [], [], options);
            end
            
            fprintf("    [DEBUG] Tube-MPC QP 最適化完了 (ExitFlag: %d)\n", exitflag);
