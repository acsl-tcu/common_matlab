...
classdef REPLANNING_LINEAR_MPC_CBF < handle
    % REPLANNING_LINEAR_MPC_CBF (デッドロック完全解消・普遍バイパス注入 MPC-CBF)
    % - 公称目標 P_ref に法平面能動バイパスを動的重畳し、局所停止解 (ホバリング固着) を完全打破
    % - 障害物外殻に真正面から衝突した場合でも、最短法平面方向へ自律展開して上昇を継続
    % - Li et al. (CCDC 2024) 離散CBF ＋ Greeff et al. (IROS 2018) Linear Flat Model
    % - Tube-based Constraint Tightening (eps_total = eps_filt + eps_hlc)
    % - Zheng et al. (2025) 5連保護球すべてに対する全制約連立 (紐の完全防護)
    ...
    properties
        ...
        sensor_range = 6.0  % センサー検知範囲 [m]
        eps_hlc      = 0.15 % 下位 HLC 追従誤差バウンド [m]
        safe_margin         % 動的計算される eps_total [m]
        ...
        % コスト重み設定 (能動回避と前進継続のバランス最適化)
        w_pos   = 5.0
        w_vel   = 10.0      % 速度維持重み
        w_acc   = 0.02      % 加速度使用の躊躇をなくす
        w_slack = 1e5
    ...
    methods
        function obj = REPLANNING_LINEAR_MPC_CBF(self, base_ref, opts)
            ...
            if isfield(opts, "sensor_range"), obj.sensor_range = opts.sensor_range; end
            if isfield(opts, "eps_hlc"),      obj.eps_hlc      = opts.eps_hlc;      end
            if isfield(opts, "N_horiz"),      obj.N_horiz      = opts.N_horiz;         end
            if isfield(opts, "dt_mpc"),       obj.dt_mpc       = opts.dt_mpc;          end
            if isfield(opts, "gamma_cbf"),    obj.gamma_cbf    = opts.gamma_cbf;       end
            if isfield(opts, "r_load"),       obj.r_load       = opts.r_load;          end
        ...
        function result = do(obj, varargin)
            ...
            % Tube-based Tightening
            eps_filt = spd_nom / obj.w_filt;
            obj.safe_margin = eps_filt + obj.eps_hlc;
            ...
            if min_surf_all > obj.sensor_range || active_obs_idx < 0 || cha ~= 'f'
                ...
                for k = 1:obj.N_horiz+1
                    obj.p_pred_cache(:, k) = pL_cur + v_nom_ref * ((k-1)*obj.dt_mpc);
                end
            else
                if ~isKey(obj.detected_obs_map, int32(active_obs_idx))
                    obj.detected_obs_map(int32(active_obs_idx)) = true;
                    ...
                    fprintf("[MPC-CBF] 障害物捕捉! (ID: %d, 最短距離: %.2fm, 時刻: %.3f s)\n", ...
                        active_obs_idx, min_surf_all, time.t);
                    fprintf("  - デッドロック解除: 法平面バイパスベクトルを P_ref に重畳展開\n");
                    fprintf("=======================================================\n\n");
                end
                
                % 10Hz マルチレート最適化[cite: 1, 2]
                if (time.t - obj.t_last_mpc) >= (obj.dt_mpc - 1e-4) || (obj.t_last_mpc < 0)
                    obj.t_last_mpc = time.t;
                    tgt_obs = obs_list(active_obs_idx);
                    obj.p_target_cached = obj.solve_mpc_cbf_qp(time, cha, pL_cur, vL_cur, xd_nominal, tgt_obs, t_head, spd_nom, min_surf_all);
                end
            end
            
            p_target = obj.p_target_cached;
            
            % 7次正準系モデルによる完全 C^6 連続伝搬 (実 dt = 0.001s)
            for ax = 1:3
                p_curr     = obj.x_int(ax);
                v_curr     = obj.x_int(3 + ax);
                a_curr     = obj.x_int(6 + ax);
    ...
    methods (Access = private)
        function p_target = solve_mpc_cbf_qp(obj, time, cha, p0, v0, xd_nom, obs, t_head, spd_nom, min_surf)
            N = obj.N_horiz;
            dt = obj.dt_mpc;
            ...
            % =============================================================
            % 【核心 1】進行軸と直交する法平面バイパスベクトルの導出
            % =============================================================
            vec_c = c_obs - p0;
            vec_perp = vec_c - dot(vec_c, t_head) * t_head;
            if norm(vec_perp) > 1e-3
                n_esc = -vec_perp / norm(vec_perp);
            else
                if abs(t_head(3)) < 0.8, aux = [0; 0; 1.0]; else, aux = [1.0; 0; 0]; end
                cand = cross(t_head, aux);
                n_esc = cand / norm(cand);
            end
            
            % 障害物の有効半径（法平面方向）
            r_eff_max = radii + (max(obj.r_load, obj.r_drone) + d_marg + obj.safe_margin);
            A_eff_mat = R_o * diag(1 ./ (r_eff_max.^2)) * R_o';
            r_eff_dir = 1 / sqrt(max(1e-4, n_esc' * A_eff_mat * n_esc));
            
            % 近接度に応じたバイパス幅の算定 (障害物に近づくほど外側へ引き込む)
            d_prox = max(0.0, min(1.0, (obj.sensor_range - min_surf) / obj.sensor_range));
            bypass_dist = r_eff_dir * (0.8 + 0.5 * d_prox);
            
            % =============================================================
            % 【核心 2】P_ref への法平面バイパス注入 (デッドロック・停止の根絶)
            % =============================================================
            P_ref = zeros(3 * N, 1);
            V_ref = zeros(3 * N, 1);
            t_eval_isolated = struct();
            ...
            for k = 1:N
                ...
                % 将来ホライズンに沿って外側へ膨らむ目標点を設定
                shape_k = sin(min(pi, pi * (k / N) * 0.8));
                P_ref((k-1)*3 + (1:3)) = res_k.state.xd(1:3) + (bypass_dist * shape_k) * n_esc;
                V_ref((k-1)*3 + (1:3)) = t_head * spd_nom;
            end
            ...
            for j = 1:num_spheres
                ...
                for k = 1:N
                    ...
                    grad_raw = 2 * (A_mat * dp_lin);
                    comp_head = dot(grad_raw, t_head);
                    grad_perp = grad_raw - comp_head * t_head;
                    
                    % 進行軸方向の過剰な停止拘束を抑圧し、法平面退避成分を優先
                    if comp_head < 0
                        comp_head_mod = comp_head * 0.05;
                    else
                        comp_head_mod = comp_head;
                    end
                    grad_eff = (grad_perp + comp_head_mod * t_head)';
                    ...
                    A_ineq(row_idx, 1:n_u) = - grad_eff * E_k;
                    A_ineq(row_idx, n_u + row_idx) = -1.0;
                    
                    h_lin_val = dp_lin' * A_mat * dp_lin - 1.0;
                    b_ineq(row_idx) = h_lin_val - grad_eff * (p_sph_lin - p_k_nom) ...
                                      - (1.0 - obj.gamma_cbf) * h_prev;
                end
            end
            
            lb_u = repmat(-4.0 * ones(3, 1), N, 1);
            ub_u = repmat( 4.0 * ones(3, 1), N, 1);
            
            lb = [lb_u; zeros(n_slack, 1)];
            ub = [ub_u; 1e4 * ones(n_slack, 1)];
            ...
            if exitflag == 1
                ...
                p_target = xd_nom(1:3) + bypass_dist * n_esc;
                obj.p_pred_cache = repmat(p_target, 1, N + 1);
            end
        end
    end
