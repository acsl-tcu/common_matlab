...
% 
%         is_braking_to_hover = false; % 減速ホバリング待機中かどうかのフラグ
%     end
% 
...
%                 else
%                     if obj.is_braking_to_hover
%                         % 【フェイルセーフ保護】減速ホバリング完了時:
%                         % 前進中の公称軌道へは絶対に戻さず、停止目標位置 (終端制御点) で完全静止ホバリングを維持
%                         xd_out = xd_nom;
%                         p_stop = obj.control_points(end, :)';
%                         xd_out(1:3) = p_stop;            % 停止位置に固定
%                         xd_out(5:7) = [0; 0; 0];         % 速度ゼロ
%                         if length(xd_out) >= 11
%                             xd_out(9:11) = [0; 0; 0];    % 加速度ゼロ
%                         end
%                         % ホバリングを保ったまま、次の安全な再計画トリガを待つ
%                     else
%                         % 通常の回避完了時: 終端条件により公称軌道の 0〜6 階微分へショックなく合流
...
%         % end
%         % =====================================================================
%         % execute_replanning: QP成否明示判定 ＆ ホバリング安全状態遷移
%         % =====================================================================
%         function execute_replanning(obj, pQ_cur, xd_nom, detection, planning_obstacles, t_now)
%             target_obs = planning_obstacles(1);
%             obj.active_threat_id = target_obs.id;
...
%             v_nom = xd_nom(5:7);
%             spd = norm(v_nom);
%             if spd < 0.1, spd = 1.0; v_nom = [0; 0; 1]; end
%             dir_nom = v_nom(:) / spd;
% 
%             % 2. 始端 0〜6階微分状態 (ホバリング中のリトライ時は現在静止状態を正確に取得)
%             init_load_state = zeros(7, 3);
%             if obj.replan_active
%                 tau_now = t_now - obj.t_start;
%                 if tau_now <= obj.t_duration
%                     for k = 0:6
%                         init_load_state(k + 1, :) = obj.eval_spline_kth(tau_now, k)';
%                     end
%                 else
%                     % ホバリング待機中: 終端静止点から完全静止微分を取得
%                     init_load_state(1, :) = obj.control_points(end, :);
%                 end
%             else
%                 init_load_state = obj.get_nominal_derivatives_at_time(t_now);
%             end
...
%             req_clearance = max(req_clearance, 1.8);
% 
%             % --- 4. 退避方向 (n_escape) 確定ロジック (幾何直交化保証版) ---
%             vec_to_center = target_obs.p_obs(:) - pQ_cur(:);
% 
%             % 進行軸に直交する相対位置成分
%             offset_perp = pQ_cur(:) - (target_obs.p_obs(:) + dot(pQ_cur(:) - target_obs.p_obs(:), dir_nom) * dir_nom);
% 
%             if norm(offset_perp) > 0.05
%                 % 横ズレがある場合: 進行軸成分を厳密に除去して直交化
%                 n_escape = offset_perp - dot(offset_perp, dir_nom) * dir_nom;
%                 n_escape = n_escape / norm(n_escape);
%             else
%                 % ほぼ真正面衝突 (ヘッドオン) の場合:
%                 % dir_nom と直交する部分空間の基底 (真横ベクトル) を代数的に取得
%                 perp_basis = null(dir_nom'); % 3x2 の直交基底行列
%                 if ~isempty(perp_basis)
%                     % 第1直交方向 (例: X軸方向) を主退避方向とする
%                     n_escape = perp_basis(:, 1);
%                     n_escape = n_escape(:) / norm(n_escape);
%                 else
%                     % フォールバック
%                     n_escape = [1; 0; 0];
...
%             end
% 
%             % 念のための幾何安全チェック: 進行軸との直交性を強制確保
%             n_escape = n_escape - dot(n_escape, dir_nom) * dir_nom;
%             if norm(n_escape) < 1e-3
%                 perp_basis = null(dir_nom');
%                 n_escape = perp_basis(:, 1);
%             end
%             n_escape = n_escape(:) / norm(n_escape);
% 
...
%             obj.t_duration = max([8.0, t_traverse + t_merge_margin]);
% 
%             % % 6. 多段階 QP 探索 (QP成功 かつ 安全検証PASS のみ採用)
%             % t_solve = tic;
%             % 
%             % % [試行 1] 通常クリアランス
%             % qp_ok = obj.plan_uniform_bspline_c6_qp(req_clearance, n_escape, init_load_state, t_now, planning_obstacles, dir_nom);
%             % if qp_ok && obj.verify_future_trajectory_safety(t_now, planning_obstacles)
%             %     obj.replan_active = true;
%             %     obj.is_braking_to_hover = false; % 通常回避
%             %     obj.last_solve_time_ms = toc(t_solve) * 1000;
%             %     obj.verify_boundary_c6_matching(init_load_state, t_now);
%             %     obj.display_detection_report(t_now, detection, req_clearance, t_impact, n_escape, "PASS");
%             %     return;
...
%             % 
%             % % [試行 2] クリアランス拡大 (1.15倍)
%             % req_boost = req_clearance * 1.15;
%             % qp_ok = obj.plan_uniform_bspline_c6_qp(req_boost, n_escape, init_load_state, t_now, planning_obstacles, dir_nom);
%             % if qp_ok && obj.verify_future_trajectory_safety(t_now, planning_obstacles)
%             %     obj.replan_active = true;
%             %     obj.is_braking_to_hover = false;
%             %     obj.last_solve_time_ms = toc(t_solve) * 1000;
%             %     obj.verify_boundary_c6_matching(init_load_state, t_now);
%             %     obj.display_detection_report(t_now, detection, req_boost, t_impact, n_escape, "BOOSTED_PASS");
%             %     return;
%             % end
%             % 
%             % % [試行 3] 逆サイド迂回 (-n_escape)
%             % qp_ok = obj.plan_uniform_bspline_c6_qp(req_clearance, -n_escape, init_load_state, t_now, planning_obstacles, dir_nom);
%             % if qp_ok && obj.verify_future_trajectory_safety(t_now, planning_obstacles)
%             %     obj.replan_active = true;
%             %     obj.is_braking_to_hover = false;
%             %     obj.last_solve_time_ms = toc(t_solve) * 1000;
%             %     obj.verify_boundary_c6_matching(init_load_state, t_now);
%             %     obj.display_detection_report(t_now, detection, req_clearance, t_impact, -n_escape, "OPPOSITE_PASS");
%             %     return;
%             % end
%             % 
%             % % 7. 【安全フェイルセーフ】厳密 C^6 減速・停止
%             % fprintf(2, '[FAIL-SAFE ACTIVATED] 全候補で安全解なし -> 厳密 C^6 減速・ホバリング待機へ移行\n');
%             % obj.plan_smooth_c6_braking_trajectory(init_load_state, t_now, dir_nom);
%             % obj.replan_active = true;
%             % obj.is_braking_to_hover = true; % ★ホバリング待機フラグをセット
%             % obj.last_solve_time_ms = toc(t_solve) * 1000;
%             % obj.verify_boundary_c6_matching(init_load_state, t_now);
%             % obj.display_detection_report(t_now, detection, req_clearance, t_impact, [0; 0; 1], "SAFE_BRAKING_HOVER");
%             % 6. 多段階 QP 探索 (QP成功 かつ 安全検証PASS のみ採用) ＆ 詳細診断出力
%             t_solve = tic;
% 
%             % [試行 1] 通常クリアランス
%             qp_ok = obj.plan_uniform_bspline_c6_qp(req_clearance, n_escape, init_load_state, t_now, planning_obstacles, dir_nom);
%             verify_ok = false;
%             if qp_ok
%                 verify_ok = obj.verify_future_trajectory_safety(t_now, planning_obstacles);
%             end
%             fprintf(2, '[DEBUG 試行1] qp_ok = %d, verify_ok = %d, disp = %.3f m, n_esc = [%.2f, %.2f, %.2f]\n', ...
%                 qp_ok, verify_ok, obj.actual_peak_displacement, n_escape(1), n_escape(2), n_escape(3));
% 
%             if qp_ok && verify_ok
%                 obj.replan_active = true;
%                 obj.is_braking_to_hover = false; % 通常回避
...
%                 verify_ok = obj.verify_future_trajectory_safety(t_now, planning_obstacles);
%             end
%             fprintf(2, '[DEBUG 試行2] qp_ok = %d, verify_ok = %d, disp = %.3f m\n', ...
%                 qp_ok, verify_ok, obj.actual_peak_displacement);
% 
%             if qp_ok && verify_ok
%                 obj.replan_active = true;
%                 obj.is_braking_to_hover = false;
...
%                 verify_ok = obj.verify_future_trajectory_safety(t_now, planning_obstacles);
%             end
%             fprintf(2, '[DEBUG 試行3] qp_ok = %d, verify_ok = %d, disp = %.3f m\n', ...
%                 qp_ok, verify_ok, obj.actual_peak_displacement);
% 
%             if qp_ok && verify_ok
%                 obj.replan_active = true;
...
%         % end
%         % =====================================================================
%         % plan_uniform_bspline_c6_qp: 成否フラグ qp_success を明示的に返却
%         % =====================================================================
%         function qp_success = plan_uniform_bspline_c6_qp(obj, req_clearance, n_escape_3d, init_load_state, t_now, planning_obstacles, dir_nom)
%             qp_success = false; % 初期値
% 
%             p = 7;
%             n_seg = 25;
%             n_cp = n_seg + p;          % 32 制御点
...
%             n_escape_3d = n_escape_3d(:);
% 
%             % 1. 始端 0〜6階微分 (M_start \ init_load_state)
%             M_start = zeros(7, 7);
%             for k = 0:6
%                 d_row = obj.eval_basis_derivatives(p + 1, 0.0, k);
%                 M_start(k + 1, :) = d_row(k + 1, 1:7);
...
% 
%                     P_rel = P_end - p_obs_k';
%                     P_local = (R_obs_k' * P_rel')';
%                     norm_coords = P_local ./ (r_safe_axes');
%                     D_eval = sqrt(sum(norm_coords.^2, 2));
%                     if any(D_eval < 1.0)
%                         all_obs_safe = false;
%                         break;
%                     end
...
%             if exitflag < 1
%                 % =================================================================
%                 % 【QP INFEASIBLE 徹底診断デバッグ出力】
%                 % =================================================================
%                 fprintf(2, '\n======================================================\n');
%                 fprintf(2, '[QP INFEASIBLE DEBUG] quadprog が解けませんでした (exitflag = %d)\n', exitflag);
%                 fprintf(2, '  ソルバー理由    : %s\n', strtrim(output.message));
%                 fprintf(2, '  req_clearance  = %.3f m\n', req_clearance);
%                 fprintf(2, '  n_escape_3d    = [%.3f, %.3f, %.3f]\n', n_escape_3d(1), n_escape_3d(2), n_escape_3d(3));
%                 fprintf(2, '  dir_nom        = [%.3f, %.3f, %.3f]\n', dir_nom(1), dir_nom(2), dir_nom(3));
%                 fprintf(2, '  内積 dot(n, d) = %.4f (※0で直交、1で平行・破綻)\n', dot(n_escape_3d, dir_nom));
%                 fprintf(2, '  T_tot          = %.3f s\n', T_tot);
%                 fprintf(2, '  制約式の総行数 = %d 行\n', size(A_ineq, 1));
% 
%                 % 始端 P_start (CP 1〜7) と 終端 P_end (CP 26〜32) の位置情報
%                 fprintf(2, '  始端位置 p_start = [%.3f, %.3f, %.3f]\n', P_start(1, 1), P_start(1, 2), P_start(1, 3));
%                 fprintf(2, '  終端位置 p_end   = [%.3f, %.3f, %.3f]\n', P_end(end, 1), P_end(end, 2), P_end(end, 3));
% 
%                 % 障害物との幾何関係
%                 for obs_idx = 1:length(planning_obstacles)
%                     o_dbg = planning_obstacles(obs_idx);
%                     p_obs_dbg = o_dbg.p_obs(:);
%                     dist_to_start = norm(P_start(1, :)' - p_obs_dbg);
%                     dist_to_end   = norm(P_end(end, :)' - p_obs_dbg);
%                     s_start = dot(P_start(1, :)' - p_obs_dbg, dir_nom);
%                     s_end   = dot(P_end(end, :)' - p_obs_dbg, dir_nom);
%                     fprintf(2, '  障害物 ID=%d (半径=[%.2f, %.2f, %.2f]):\n', ...
%                         o_dbg.id, o_dbg.radii_obs(1), o_dbg.radii_obs(2), o_dbg.radii_obs(3));
%                     fprintf(2, '    始端からの距離: %.2f m (進行軸投影 s = %.2f m)\n', dist_to_start, s_start);
%                     fprintf(2, '    終端からの距離: %.2f m (進行軸投影 s = %.2f m)\n', dist_to_end, s_end);
%                 end
% 
%                 % H行列の条件数（数値的特異性のチェック）
%                 fprintf(2, '  H Hessian 条件数 = %.2e (※1e+12超でランク落ち・数値破綻)\n', cond(H));
%                 fprintf(2, '======================================================\n\n');
% 
%                 qp_success = false;
%                 return;
...
%                     margin_Q = dQ - obj.r_drone - obj.clearance_margin;
%                     if margin_Q <= 0
%                         fprintf(2, '[REJECT UAV] t_sample = %.3f s, margin = %.3f m (dQ=%.3f, r_drone=%.3f, cl_margin=%.3f)\n', ...
%                             t_eval, margin_Q, dQ, obj.r_drone, obj.clearance_margin);
%                         is_safe = false;
%                         return;
%                     end
% 
...
%                     margin_L = dL - obj.r_load - obj.clearance_margin;
%                     if margin_L <= 0
%                         fprintf(2, '[REJECT LOAD] t_sample = %.3f s, margin = %.3f m (dL=%.3f, r_load=%.3f, cl_margin=%.3f)\n', ...
%                             t_eval, margin_L, dL, obj.r_load, obj.clearance_margin);
%                         is_safe = false;
%                         return;
%                     end
% 
...
%                         margin_C = dC - r_c_sph - obj.clearance_margin;
%                         if margin_C <= 0
%                             fprintf(2, '[REJECT CABLE (sp=%d/%d)] t_sample = %.3f s, margin = %.3f m (dC=%.3f, r_sph=%.3f, cl_margin=%.3f)\n', ...
%                                 sp, n_spheres, margin_C, dC, r_c_sph, obj.clearance_margin);
%                             is_safe = false;
%                             return;
%                         end
%                     end
...
% 
%         % =====================================================================
%         % is_nominal_recovery_safe: 復帰先の公称軌道における干渉スキャン (完全球列統一版)
%         % UAV, Load, および索全域 (球列完全被覆) を回避時と完全同一基準で判定
%         % =====================================================================
%         function is_safe = is_nominal_recovery_safe(obj, t_now, obs_list)
%             is_safe = true;
%             N_lookahead = 20; % 復帰区間の時間解像度を確保
%             t_scan = linspace(t_now + obj.t_duration, t_now + obj.t_duration + 2.5, N_lookahead);
% 
%             % システム統一の重力加速度ベクトル (3x1 列ベクトル)
%             g_vec = [0; 0; obj.gravity];
%             g_vec = g_vec(:);
% 
%             % 索球列パラメータ (verify_future_trajectory_safety と完全統一)
%             r_c_sph = obj.r_cable_sphere;
%             n_spheres = max(2, ceil(obj.L_cable / (2.0 * r_c_sph)) + 1);
%             s_ratios = linspace(0.0, 1.0, n_spheres);
% 
...
% 
%                 % 1. 荷物の位置および加速度の取得 (3x1 列ベクトル)
%                 pL_nom = nom_res.state.xd(1:3);
%                 pL_nom = pL_nom(:);
% 
%                 if length(nom_res.state.xd) >= 11
...
% 
%                 % 3. 索上の全球列座標を一括展開 (3 x n_spheres)
%                 cable_pts_nom = (1 - s_ratios) .* pL_nom + s_ratios .* pQ_nom;
% 
%                 % 4. 障害物干渉判定 (UAV, Load, 索球列完全スキャン)
%                 for obs_idx = 1:length(obs_list)
%                     o = obs_list(obs_idx);
% 
%                     if isfield(o, 'p_obs')
...
%                     if dL - obj.r_load - obj.clearance_margin <= 0
%                         is_safe = false;
%                         return;
%                     end
% 
...
% 
%         % =====================================================================
%         % plan_smooth_c6_braking_trajectory: 厳密代数 C^6 減速・安全ホバリング軌道
%         % 始端 0〜6階微分を完全一致させ、進行方向に沿って滑らかに減速完全静止
%         % =====================================================================
%         function plan_smooth_c6_braking_trajectory(obj, init_load_state, t_now, dir_nom)
%             p = 7;
%             n_seg = 20;
%             n_cp = n_seg + p; % 27 制御点
% 
%             % 3.5 秒かけて滑らかに減速停止
%             T_brake = 3.5;
%             obj.t_duration = T_brake;
%             obj.t_start = t_now; % 時系列の開始時刻を同期
%             obj.spline_degree = p;
%             obj.num_segments = n_seg;
%             obj.build_clamped_uniform_knots(n_seg, p, T_brake);
% 
%             % 1. 始端 0〜6階微分 (現在状態と完全連続)
%             M_start = zeros(7, 7);
%             for k = 0:6
%                 d_row = obj.eval_basis_derivatives(p + 1, 0.0, k);
%                 M_start(k + 1, :) = d_row(k + 1, 1:7);
...
%             P_start = M_start \ init_load_state(1:7, :);
% 
%             % 2. 終端 0〜6階微分 (安全停止・ホバリング状態)
%             % dir_nom に沿った前進速度成分を取り出し、進行軸に沿ってまっすぐ安全制動
%             p_cur = init_load_state(1, :)';
%             v_cur = init_load_state(2, :)';
%             dir_unit = dir_nom(:) / max(1e-3, norm(dir_nom));
%             v_forward = max(0.0, dot(v_cur, dir_unit)); % 進行方向の速度成分
% 
%             % 制動停止目標位置: 進行方向にのみ少し進んで静止
%             p_stop = p_cur + (0.4 * v_forward * T_brake) * dir_unit;
% 
%             stop_state = zeros(7, 3);
%             stop_state(1, :) = p_stop'; % 0階: 停止位置
%             % 1階(速度)〜6階(Pop) はすべて 0 (完全静止)
% 
%             M_end = zeros(7, 7);
%             for k = 0:6
%                 d_row_end = obj.eval_basis_derivatives(n_cp, T_brake, k);
%                 M_end(k + 1, :) = d_row_end(k + 1, (end - 6):end);
%             end
%             P_end = M_end \ stop_state(1:7, :);
% 
%             % 3. 中間制御点の滑らかな線形補間
%             n_mid = n_cp - 14;
%             P_mid = zeros(n_mid, 3);
%             P_last_start = P_start(end, :);
%             P_first_end  = P_end(1, :);
%             for d = 1:3
%                 v_lin = linspace(P_last_start(d), P_first_end(d), n_mid + 2);
%                 P_mid(:, d) = v_lin(2:end-1)';
%             end
% 
%             obj.control_points = [P_start; P_mid; P_end];
%             obj.actual_peak_displacement = 0.0;
%         end
% 
...
classdef REPLANNING_BSPLINE < handle
    ...
    methods (Access = private)
        ...
        function plan_uniform_bspline_c6_qp(obj, req_clearance, n_escape_3d, init_load_state, t_now, planning_obstacles, dir_nom)
            ...
            for obs_idx = 1:length(planning_obstacles)
                cur_obs = planning_obstacles(obs_idx);
                n_constraints_before = size(A_ineq, 1);
                p_obs = cur_obs.p_obs(:);
                R_obs = cur_obs.R_obs;
                r_obs = cur_obs.radii_obs(:);
                ...
                n_constraints_after = size(A_ineq, 1);
                n_constraints_obs = n_constraints_after - n_constraints_before;

                fprintf(2, '[QP OBSTACLE %d]\n', obs_idx);
                fprintf(2, '  ID                  = %d\n', cur_obs.id);
                fprintf(2, '  center              = [%.3f %.3f %.3f]\n', ...
                    p_obs(1), p_obs(2), p_obs(3));
                fprintf(2, '  radii               = [%.3f %.3f %.3f]\n', ...
                    r_obs(1), r_obs(2), r_obs(3));
                fprintf(2, '  L_along             = %.3f m\n', L_along);
                fprintf(2, '  n_escape            = [%.3f %.3f %.3f]\n', ...
                    n_esc(1), n_esc(2), n_esc(3));
                fprintf(2, '  s_enter             = %.3f m\n', s_enter);
                fprintf(2, '  s_exit              = %.3f m\n', s_exit);
                fprintf(2, '  generated constraints = %d\n', n_constraints_obs);
            end
            % =================================================================
            % QP事前診断
            % =================================================================
            fprintf('\n');
            fprintf(2, '=============================================================\n');
            fprintf(2, '[QP PRE-DIAGNOSTIC]\n');
            fprintf(2, '=============================================================\n');

            fprintf(2, '  T_tot             = %.4f s\n', T_tot);
            fprintf(2, '  req_clearance     = %.4f m\n', req_clearance);
            fprintf(2, '  n_free            = %d\n', n_free);
            fprintf(2, '  decision variables= %d\n', length(f));
            fprintf(2, '  inequality count  = %d\n', size(A_ineq,1));

            fprintf(2, '  H size            = %dx%d\n', size(H,1), size(H,2));
            fprintf(2, '  rank(H)           = %d\n', rank(H));

            if isempty(A_ineq)
                fprintf(2, '  WARNING: A_ineq is EMPTY\n');
            else
                fprintf(2, '  rank(A_ineq)      = %d\n', rank(A_ineq));
                fprintf(2, '  max|A_ineq|       = %.4e\n', max(abs(A_ineq(:))));
                fprintf(2, '  max|b_ineq|       = %.4e\n', max(abs(b_ineq)));
            end

            fprintf(2, '  min(P_start distance to obstacle constraints) will be checked below.\n');
            fprintf(2, '=============================================================\n');
            % 5. QP 求解 (内点法二次計画法)
            opts = optimoptions('quadprog', 'Display', 'off', 'Algorithm', 'interior-point-convex');
            % =================================================================
            ...
            if ~isempty(A_ineq)
                ...
                fprintf(2, '\n[QP FEASIBILITY-ONLY TEST]\n');

                n_var = size(A_ineq, 2);

                H_feas = eye(n_var) * 1e-8;
                f_feas = zeros(n_var, 1);

                opts_feas = optimoptions('quadprog', ...
                    'Display', 'off', ...
                    'Algorithm', 'interior-point-convex');

                [~, ~, exitflag_feas, output_feas] = quadprog( ...
                    H_feas, f_feas, ...
                    A_ineq, b_ineq, ...
                    [], [], [], [], [], opts_feas);

                fprintf(2, '  feasibility exitflag = %d\n', exitflag_feas);

                if isfield(output_feas, 'message')
                    fprintf(2, '  feasibility message = %s\n', output_feas.message);
                end

                if exitflag_feas >= 1
                    fprintf(2, '  RESULT = CONSTRAINTS ARE FEASIBLE\n');
                else
                    fprintf(2, '  RESULT = CONSTRAINTS ARE INFEASIBLE OR NUMERICALLY FAILED\n');
                end
            end
            % [X_mid, ~, exitflag] = quadprog(H, f, A_ineq, b_ineq, [], [], [], [], [], opts);
            [X_mid, fval, exitflag, output] = quadprog( ...
                H, f, A_ineq, b_ineq, [], [], [], [], [], opts);
            if exitflag >= 1
                constraint_residual = A_ineq * X_mid - b_ineq;

                fprintf(2, '\n[QP CONSTRAINT CHECK]\n');
                fprintf(2, '  max(A*x-b) = %.6e\n', max(constraint_residual));
                fprintf(2, '  min(A*x-b) = %.6e\n', min(constraint_residual));

                for k = 1:length(constraint_residual)
                    fprintf(2, '  constraint %2d : A*x-b = %+ .6e\n', ...
                        k, constraint_residual(k));
                end
            end
            % =================================================================
            % QP求解結果診断
            ...
            fprintf(2, '\n');
            fprintf(2, '=============================================================\n');
            fprintf(2, '[QP SOLVER RESULT]\n');
            fprintf(2, '=============================================================\n');

            fprintf(2, '  exitflag = %d\n', exitflag);
            fprintf(2, '  fval     = %.6e\n', fval);
            ...
            if isfield(output, 'iterations')
                fprintf(2, '  iterations = %d\n', output.iterations);
            end

            if isfield(output, 'algorithm')
                fprintf(2, '  algorithm  = %s\n', output.algorithm);
            end

            if exitflag >= 1
                fprintf(2, '  STATUS = QP SOLVED\n');
            elseif exitflag == -2
                fprintf(2, '  STATUS = QP INFEASIBLE\n');
            elseif exitflag == -3
                fprintf(2, '  STATUS = QP UNBOUNDED\n');
            else
                fprintf(2, '  STATUS = QP FAILED / NUMERICAL ISSUE\n');
            end
            ...
            fprintf(2, '\n[QP CONTROL POINT CHECK]\n');

            for j = 1:n_free
                P_test = P_mid(j, :)';

                s_test = dot(P_test - p_obs, dir_nom);

                if (s_test >= s_enter) && (s_test <= s_exit)

                    side_dist = dot(n_esc, P_test - p_obs);

                    fprintf(2, ...
                        '  CP %2d: P=[%+.3f %+.3f %+.3f], s=%+.3f, side=%+.3f\n', ...
                        j, ...
        ...
        function is_safe = verify_future_trajectory_safety(obj, t_now, obs_list)
            ...
            for i = 1:N_samples
                tau_i = taus(i);
                t_fut = t_now + tau_i;
                fprintf(2, '\n[SAFETY VERIFY] checking tau = %.3f s, t = %.3f s\n', ...
                    tau_i, t_fut);
                nom_res = obj.base_ref.do(struct('t', t_fut, 'dt', 0.025), 'f');
                xd_nom_i = nom_res.state.xd;
                if length(xd_nom_i) < 28, xd_nom_i = [xd_nom_i; zeros(28 - length(xd_nom_i), 1)]; end
                ...
                pQ_fut = pL_fut + obj.L_cable * thrust_dir;
                fprintf(2, ...
                    '[TRAJ CHECK] tau=%.3f | pL=[%.3f %.3f %.3f] | aL=[%.3f %.3f %.3f] | thrust=[%.3f %.3f %.3f] | pQ=[%.3f %.3f %.3f]\n', ...
                    tau_i, ...
                    pL_fut(1), pL_fut(2), pL_fut(3), ...
                    xd_fut(9), xd_fut(10), xd_fut(11), ...
                    thrust_dir(1), thrust_dir(2), thrust_dir(3), ...
                    pQ_fut(1), pQ_fut(2), pQ_fut(3));
                ...
                for obs_idx = 1:length(obs_list)
                    ...
                    % 1. UAV
                    [dQ, ~, ~, ~] = obj.point_ellipsoid_signed_distance(pQ_fut, obs_parsed); 
                    clearance_Q = dQ - obj.r_drone;
                    
                    if clearance_Q <= 0
                        fprintf(2, '\n=============================================================\n');
                        fprintf(2, '[SAFETY VERIFY FAILURE] UAV\n');
                        fprintf(2, '  sample       = %d / %d\n', i, N_samples);
                        fprintf(2, '  tau          = %.6f s\n', tau_i);
                        fprintf(2, '  t_abs        = %.6f s\n', t_fut);
                        fprintf(2, '  obstacle ID  = %d\n', o.id);
                        fprintf(2, '  pQ           = [%.6f %.6f %.6f]\n', ...
                            pQ_fut(1), pQ_fut(2), pQ_fut(3));
                        fprintf(2, '  dQ           = %.6f m\n', dQ);
                        fprintf(2, '  UAV radius   = %.6f m\n', obj.r_drone);
                        fprintf(2, '  clearance    = %.6f m\n', clearance_Q);
                        fprintf(2, '  threshold    = %.6f m\n', obj.r_drone);
                        fprintf(2, '=============================================================\n');
                    ...
                    % 2. 荷物
                    [dL, ~, ~, ~] = obj.point_ellipsoid_signed_distance(pL_fut, obs_parsed); 
                    clearance_L = dL - obj.r_load;
                    
                    if clearance_L <= 0
                        fprintf(2, '\n=============================================================\n');
                        fprintf(2, '[SAFETY VERIFY FAILURE] LOAD\n');
                        fprintf(2, '  sample       = %d / %d\n', i, N_samples);
                        fprintf(2, '  tau          = %.6f s\n', tau_i);
                        fprintf(2, '  t_abs        = %.6f s\n', t_fut);
                        fprintf(2, '  obstacle ID  = %d\n', o.id);
                        fprintf(2, '  pL           = [%.6f %.6f %.6f]\n', ...
                            pL_fut(1), pL_fut(2), pL_fut(3));
                        fprintf(2, '  dL           = %.6f m\n', dL);
                        fprintf(2, '  load radius  = %.6f m\n', obj.r_load);
                        fprintf(2, '  clearance    = %.6f m\n', clearance_L);
                        fprintf(2, '  threshold    = %.6f m\n', obj.r_load);
                        fprintf(2, '=============================================================\n');
                    ...
                    % 3. 索球列
                    for sp = 1:n_spheres 
                        [dC, ~, ~, ~] = obj.point_ellipsoid_signed_distance( ...
                            cable_pts_fut(:, sp), obs_parsed); 
                    
                        clearance_C = dC - r_c_sph;
                    
                        if clearance_C <= 0
                            fprintf(2, '\n=============================================================\n');
                            fprintf(2, '[SAFETY VERIFY FAILURE] CABLE\n');
                            fprintf(2, '  sample       = %d / %d\n', i, N_samples);
                            fprintf(2, '  tau          = %.6f s\n', tau_i);
                            fprintf(2, '  t_abs        = %.6f s\n', t_fut);
                            fprintf(2, '  obstacle ID  = %d\n', o.id);
                            fprintf(2, '  cable sphere = %d / %d\n', sp, n_spheres);
                            fprintf(2, '  s_ratio      = %.6f\n', s_ratios(sp));
                            fprintf(2, '  pC           = [%.6f %.6f %.6f]\n', ...
                                cable_pts_fut(1, sp), ...
                                cable_pts_fut(2, sp), ...
                                cable_pts_fut(3, sp));
                            fprintf(2, '  dC           = %.6f m\n', dC);
                            fprintf(2, '  sphere radius= %.6f m\n', r_c_sph);
                            fprintf(2, '  clearance    = %.6f m\n', clearance_C);
                            fprintf(2, '  threshold    = %.6f m\n', r_c_sph);
                            fprintf(2, '=============================================================\n');
