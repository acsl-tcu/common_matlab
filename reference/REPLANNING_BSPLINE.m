classdef REPLANNING_BSPLINE < handle
    ...
    methods (Access = private)
        ...
        function execute_replanning(obj, pQ_cur, xd_nom, detection, planning_obstacles, t_now)
            ...
            % --- 7. 微分平坦性クローズドループ安全検証 ＆ 採用ガード ---
            % [第1段階検証]
            [traj_verified, worst_violation, fail_info] = obj.verify_future_trajectory_safety(t_now, planning_obstacles);
            ...
            if traj_verified
                ...
                % 1回目で不合格 -> 侵入原因と不足深さをログ出力し、平坦性フィードバック補正
                delta_clearance = worst_violation + 0.15; % 実測不足深さ + 15cm 保証マージン
                req_clearance_boost = req_clearance + delta_clearance;
                
                fprintf(2, '\n[STAGE 2 FEEDBACK] %s 侵入検知 (不足深さ: %.4fm at t=%.3fs) -> クリアランスを +%.4fm 補正して再QP実行\n', ...
                    fail_info.type, worst_violation, fail_info.t_abs, delta_clearance);
                
                % 第2回 QP再計画
                obj.plan_uniform_bspline_c6_qp(req_clearance_boost, n_escape, init_load_state, t_now, planning_obstacles, dir_nom);
                
                % [第2段階検証] Boosted 軌道を再検証 (全ログ出力)
                [boosted_verified, worst_violation_2, fail_info_2] = obj.verify_future_trajectory_safety(t_now, planning_obstacles);
                
                if boosted_verified
                    % 2回目で合格 -> 採用
                    obj.replan_active = true;
                    obj.verify_boundary_c6_matching(init_load_state, t_now);
                    obj.display_detection_report(t_now, detection, req_clearance_boost, t_impact, n_escape, "CLOSED_LOOP_PASS");
                else
                    fprintf(2, '[STAGE 2 WARNING] 補正後も残余侵入あり: %.4fm (%s) -> 安全確保のため最大退避軌道を採用\n', ...
                        worst_violation_2, fail_info_2.type);
                    obj.replan_active = true;
                    obj.verify_boundary_c6_matching(init_load_state, t_now);
        ...
        function plan_uniform_bspline_c6_qp(obj, req_clearance, n_escape_3d, init_load_state, t_now, planning_obstacles, dir_nom)
            ...
            % =================================================================
            % 4. 微分平坦性 3者統合支持超平面スタック (Load, UAV, Cable in QP)
            %    任意の楕円体姿勢・任意進入角に対応する厳密幾何凸制約
            % =================================================================
            A_ineq = [];
            b_ineq = [];
            
            % ノット区間に応じた評価時刻ベクトル (自由制御点に対応する代表時刻)
            dt_step = T_tot / n_seg;
            t_samples = dt_step * (idx_free - (p / 2));
            t_samples = max(0.05 * T_tot, min(0.95 * T_tot, t_samples));
            g_vec = [0; 0; obj.gravity];
            ...
            for obs_idx = 1:length(planning_obstacles)
                ...
                for j = 1:n_free
                    ...
                    % 1. 公称状態と微分平坦性推力方向 q_0(t) の取得
                    nom_res_j = obj.base_ref.do(struct('t', t_eval, 'dt', 0.025), 'f');
                    pL_nom_j  = nom_res_j.state.xd(1:3); pL_nom_j = pL_nom_j(:);
                    if length(nom_res_j.state.xd) >= 11
                        aL_nom_j = nom_res_j.state.xd(9:11); aL_nom_j = aL_nom_j(:);
                    else
                        aL_nom_j = zeros(3, 1);
                    end
                    
                    acc_tot = aL_nom_j + g_vec;
                    if norm(acc_tot) > 1e-3, q_0 = acc_tot / norm(acc_tot); else, q_0 = [0; 0; 1]; end
                    
                    % 2. B-spline基底関数の行ベクトル (1 x n_free) を抽出
                    idx_span = obj.find_knot_span(min(T_tot - 1e-6, tau_j));
                    ders_j = obj.eval_basis_derivatives(idx_span, min(T_tot - 1e-6, tau_j), 0);
                    b_full = zeros(1, n_cp);
                    b_full((idx_span - p):idx_span) = ders_j(1, :);
                    
                    % 境界確定済みの P_start, P_end による既知項の分離
                    b_start = b_full(1:7);
                    b_mid   = b_full(idx_free);
                    b_end   = b_full((n_cp - 6):n_cp);
                    p_fixed_offset = (b_start * P_start + b_end * P_end)'; % (3x1)
                    
                    % 3. 3者 (Load, UAV, Cable中点) に対する厳密支持超平面の生成
                    entity_offsets = [0.0, 1.0, 0.5] * obj.L_cable; % q_0 方向へのシフト量
                    entity_radii   = [obj.r_load, obj.r_drone, obj.r_cable_sphere];
                    entity_names   = ["LOAD", "UAV", "CABLE"]; % ★ここに追加
                    
                    for ent = 1:3
                        s_len = entity_offsets(ent);
                        r_ent = entity_radii(ent);
                        
                        % 公称参照位置
                        p_ent_nom = pL_nom_j + s_len * q_0;
                        
                        % 楕円体外郭との厳密最短距離および外向き法線の計算
                        [d_signed, ~, closest_local, ~] = obj.point_ellipsoid_signed_distance(p_ent_nom, obs_struct);
                        
                        % 影響圏内にある場合のみ支持超平面制約を生成
                        influence_dist = 4.0 + req_clearance;
                        if d_signed < influence_dist
                            closest_world = obs_struct.center + obs_struct.R * closest_local;
                            
                            % 楕円体外郭での幾何法線
                            n_loc = closest_local ./ (obs_struct.radii.^2);
                            n_geom = obs_struct.R * (n_loc / norm(n_loc));
                            
                            % --- [進行軸干渉の除去: 側方退避法線への射影] ---
                            % 進行軸と逆向きの法線成分は進行を阻み、制御点を横に吹き飛ばす原因になるため除去
                            n_lat = n_geom - dot(n_geom, dir_nom) * dir_nom;
                            if norm(n_lat) > 0.05
                                n_out = n_lat / norm(n_lat);
                            else
                                % 真正面衝突（特異点）時は指定の退避方向を採用
                                n_out = n_escape_3d;
                            end
                            
                            % 厳密支持超平面の安全境界点
                            safe_clearance = r_ent + req_clearance;
                            p_plane_boundary = closest_world + safe_clearance * n_out;
                            
                            % 不等式定式化: -n_out' * p_entity <= -n_out' * p_plane_boundary
                            row_constraint = zeros(1, 3 * n_free);
                            for dim = 1:3
                                idx_d = (dim - 1) * n_free;
                                row_constraint(idx_d + (1:n_free)) = -n_out(dim) * b_mid;
                            end
                            
                            rhs_val = -dot(n_out, p_plane_boundary - p_fixed_offset - s_len * q_0);
                            % === [DEBUG LOG 1: 生成制約の幾何学的内訳] ===
                            fprintf(2, '[DEBUG CONSTRAINT GEN] tau=%.3f s | Free CP %2d (CP %2d) | ent=%s\n', ...
                                tau_j, j, idx_free(j), entity_names(ent));
                            fprintf(2, '  p_ent_nom   = [%+.3f %+.3f %+.3f]\n', ...
                                p_ent_nom(1), p_ent_nom(2), p_ent_nom(3));
                            fprintf(2, '  d_signed    = %+.4f m (influence_dist=%.4f)\n', ...
                                d_signed, influence_dist);
                            fprintf(2, '  n_out       = [%+.4f %+.4f %+.4f] (norm=%.4f)\n', ...
                                n_out(1), n_out(2), n_out(3), norm(n_out));
                            fprintf(2, '  p_boundary  = [%+.3f %+.3f %+.3f]\n', ...
                                p_plane_boundary(1), p_plane_boundary(2), p_plane_boundary(3));
                            fprintf(2, '  b_mid: max=%.4e, min=%.4e, nnz=%d\n', ...
                                max(b_mid), min(b_mid), nnz(b_mid > 1e-6));
                            fprintf(2, '  p_fixed_off = [%+.3e %+.3e %+.3e]\n', ...
                                p_fixed_offset(1), p_fixed_offset(2), p_fixed_offset(3));
                            fprintf(2, '  rhs_val(b)  = %+.4e\n', rhs_val);
                            % ==============================================
                            
                            A_ineq = [A_ineq; row_constraint];
                ...
                fprintf(2, '  generated constraints = %d (Load+UAV+Cable 平坦性統合)\n', n_constraints_obs);
            end
            % =================================================================
            % QP事前診断
            % =================================================================
            ...
            % === [DEBUG LOG 2: 境界確定制御点と公称軌道の診断] ===
            fprintf(2, '\n=============================================================\n');
            fprintf(2, '[DEBUG QP INPUTS: BOUNDARY & NOMINAL CONTROL POINTS]\n');
            fprintf(2, '=============================================================\n');
            fprintf(2, '--- P_start (CP 1〜7: 始端固定) ---\n');
            for k = 1:7
                fprintf(2, '  CP %2d: [%+10.4f %+10.4f %+10.4f]\n', ...
                    k, P_start(k, 1), P_start(k, 2), P_start(k, 3));
            end
            fprintf(2, '--- P_end (CP 26〜32: 終端公称合流) ---\n');
            for k = 1:7
                fprintf(2, '  CP %2d: [%+10.4f %+10.4f %+10.4f]\n', ...
                    n_cp - 7 + k, P_end(k, 1), P_end(k, 2), P_end(k, 3));
            end
            fprintf(2, '--- P_nom_free (自由制御点の公称値) ---\n');
            for k = 1:n_free
                fprintf(2, '  Free %2d (CP %2d): [%+10.4f %+10.4f %+10.4f]\n', ...
                    k, idx_free(k), P_nom_free(k, 1), P_nom_free(k, 2), P_nom_free(k, 3));
            end
            fprintf(2, '--- 目的関数診断 ---\n');
            fprintf(2, '  cond(H)     = %.4e\n', cond(H));
            fprintf(2, '  min/max eig = %.4e / %.4e\n', min(eig(H)), max(eig(H)));
            fprintf(2, '  norm(f)     = %.4e\n', norm(f));
            fprintf(2, '  f(1:3)      = [%+.4e %+.4e %+.4e]\n', f(1), f(n_free+1), f(2*n_free+1));
            fprintf(2, '=============================================================\n\n');
            % ===================================================
            % 5. QP 求解 (内点法二次計画法)
            ...
            % === [DEBUG LOG 3: QP解ベクトルの生データ診断] ===
            if exitflag >= 1
                fprintf(2, '\n=============================================================\n');
                fprintf(2, '[DEBUG QP RAW SOLUTION X_mid]\n');
                fprintf(2, '=============================================================\n');
                fprintf(2, '  X_mid min/max/norm = %.4e / %.4e / %.4e\n', ...
                    min(X_mid), max(X_mid), norm(X_mid));
                for k = 1:n_free
                    fprintf(2, '  Free CP %2d (CP %2d): X=[%+10.4f] Y=[%+10.4f] Z=[%+10.4f]\n', ...
                        k, idx_free(k), X_mid(k), X_mid(n_free + k), X_mid(2*n_free + k));
                end
                fprintf(2, '=============================================================\n\n');
            end
            % ================================================
            ...
            for j = 1:n_free
                P_test = P_mid(j, :)';
                % 第1優先障害物中心に対する相対位置ベクトル
                p_rel_test = P_test - p_obs;
                dist_to_center = norm(p_rel_test);
                s_along = dot(p_rel_test, dir_nom);
                side_dist = dot(n_escape_3d, p_rel_test);
                
                fprintf(2, ...
                    '  CP %2d: P=[%+.3f %+.3f %+.3f] | dist_center=%.3fm | s_along=%+.3fm | side=%+.3fm\n', ...
                    j, ...
                    P_test(1), P_test(2), P_test(3), ...
                    dist_to_center, s_along, side_dist);
            end
        ...
        % (ログ完全保持 ＆ 微分平坦性フィードバック拡張版)
        % =====================================================================
        function [is_safe, worst_violation, fail_info] = verify_future_trajectory_safety(obj, t_now, obs_list)
            is_safe = true;
            worst_violation = 0.0;
            fail_info = struct('type', 'NONE', 'tau', NaN, 't_abs', NaN, 'obs_id', NaN, ...
                               'p_pos', zeros(3,1), 'd_signed', NaN, 'clearance', NaN, 'dist_deficit', 0.0);
            ...
            for i = 1:N_samples
                ...
                for obs_idx = 1:length(obs_list)
                    ...
                    if clearance_Q <= 0
                        deficit_Q = -clearance_Q;
                        if deficit_Q > worst_violation
                            worst_violation = deficit_Q;
                            fail_info = struct('type', 'UAV', 'tau', tau_i, 't_abs', t_fut, 'obs_id', o.id, ...
                                               'p_pos', pQ_fut, 'd_signed', dQ, 'clearance', clearance_Q, 'dist_deficit', deficit_Q);
                        end
                        is_safe = false;
                    ...
                    if clearance_L <= 0
                        deficit_L = -clearance_L;
                        if deficit_L > worst_violation
                            worst_violation = deficit_L;
                            fail_info = struct('type', 'LOAD', 'tau', tau_i, 't_abs', t_fut, 'obs_id', o.id, ...
                                               'p_pos', pL_fut, 'd_signed', dL, 'clearance', clearance_L, 'dist_deficit', deficit_L);
                        end
                        is_safe = false;
                    ...
                    for sp = 1:n_spheres 
                        ...
                        if clearance_C <= 0
                            deficit_C = -clearance_C;
                            if deficit_C > worst_violation
                                worst_violation = deficit_C;
                                fail_info = struct('type', 'CABLE', 'tau', tau_i, 't_abs', t_fut, 'obs_id', o.id, ...
                                                   'p_pos', cable_pts_fut(:, sp), 'd_signed', dC, 'clearance', clearance_C, 'dist_deficit', deficit_C);
                            end
                            is_safe = false;
