classdef REPLANNING_POLYTOPE_MPC < handle
    % REPLANNING_POLYTOPE_MPC
    % 案2: 分散型ロバストMPCによるポリトープ安全領域の順次再生成 (Tartaglione 2017)
    % 飛行禁止区域を回避する凸な安全ポリトープ(コリドー)を定義し、Pontryagin差による
    % RPIマージンを適用した完全線形MPC(QP)を解いて安全なウェイポイント列を生成する。
    % その後、13次多項式を用いて6階微分まで平滑化フィッティングを行う。
    ...
    methods
        function obj = REPLANNING_POLYTOPE_MPC(self, base_ref, opts)
            arguments
                self
                base_ref
                opts = struct()
        ...
        function result = do(obj, varargin)
            ...
            if cha == 'f' && ~obj.replan_done && ~obj.replan_active
                ...
                if min_dist <= obj.trigger_dist
                    fprintf("\n=======================================================\n");
                    fprintf("[POLYTOPE MPC] 障害物接近検知! (min_dist=%.2fm, t=%.3f s)\n", min_dist, time.t);
                    obj.t_start = time.t;
                    ...
                    fprintf("[POLYTOPE MPC] 第1段階: ポリトープ制約付き線形MPC (QP) の実行...\n");
                    W_opt = obj.plan_polytope_mpc_waypoints(p0_ref, dir_nom, spd, total_dist);
                    
                    fprintf("[POLYTOPE MPC] 第2段階: 13次多項式 Mellinger 平滑化フィッティング...\n");
            ...
            if obj.replan_active
                ...
                if tau <= obj.t_duration
                    ...
                    if ~obj.replan_done
                        fprintf("[POLYTOPE MPC] 障害物クリア・C^6シームレス合流 (t=%.3f s)\n\n", time.t);
    ...
    methods (Access = private)
        function W_opt = plan_polytope_mpc_waypoints(obj, p0, dir_nom, spd, total_dist)
            % Nステップの線形MPCを完全な二次計画法(QP)として定式化する
            N = 8;
            dt = obj.t_duration / N;
            
            % 変数は V = [v_1; v_2; ...; v_N] (3N次元ベクトル)
            num_vars = 3 * N;
            ...
            % --- 1. Pontryagin 差による安全領域ポリトープの定義 ---
            Delta_tube = obj.d_max / obj.K_e;
            R_base = obj.obs_radius + obj.safe_margin + obj.L_cable * 0.5;
            R_tight = R_base + Delta_tube;
            ...
            % 障害物を回避する安全領域（超平面のこちら側）
            % a_h^T r <= b_h  =>  -dir_normal^T r <= -dir_normal^T (p_obs + dir_normal * R_tight)
            a_h = -dir_normal;
            p_plane = obj.obs_center + dir_normal * R_tight;
            b_h = dot(a_h, p_plane);
            
            % --- 2. QP 目的関数 (H, f) の構築 ---
            % 各ステップ k の位置 r_k は V に対して線形に表せる:
            % r_k = p0 + dt * sum_{i=1}^k v_i
            ...
            for k = 1:N
                ...
                % 追従コスト (r_k - p_ref_k)^T W (r_k - p_ref_k)
                % = (E_k V + p0 - p_ref_k)^T W (E_k V + p0 - p_ref_k)
                diff_const = p0 - p_ref_k;
                H = H + w_pos * (E_k' * E_k);
                f = f + w_pos * (E_k' * diff_const);
                
                % 速度平滑化コスト (v_k - v_ref)^T W (v_k - v_ref)
                idx_k = (k-1)*3 + 1 : k*3;
            ...
            % 二次形式 1/2 V^T (2H) V + V^T f なので、H を2倍
            H = 2 * H;
            
            % --- 3. 不等式制約 (A_ineq * V <= b_ineq) ---
            % 各 r_k がポリトープ領域内にいること: a_h^T r_k <= b_h
            A_ineq = zeros(N, num_vars);
            b_ineq = zeros(N, 1);
            ...
            for k = 1:N
                ...
                % a_h^T (p0 + E_k * V) <= b_h
                % (a_h^T E_k) V <= b_h - a_h^T p0
                A_ineq(k, :) = a_h' * E_k;
                b_ineq(k) = b_h - dot(a_h, p0);
            end
            ...
            if exitflag ~= 1
                fprintf("    [WARNING] ポリトープMPC QP が最適解に到達しませんでした(exitflag=%d)\n", exitflag);
            ...
            fprintf("    - 線形MPC (QP) によるウェイポイント生成完了 (N=%d)\n", N);
        end
