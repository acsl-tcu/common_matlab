...
%     % REPLANNING_SMOOTH_APF
%     % 論文ベースの「本気の人工ポテンシャル法 (APF)」リプランナ
%     % 
%     % 参考文献:
%     % 1. O. Khatib (1986) - APFの大元 (引力と斥力の定式化)
%     % 2. R. Funada et al. - 楕円体障害物の距離計算
%     % 3. M. Cohen et al. - 力の発散を防ぐ Smooth Saturation
%     % 4. D. Tscholl et al. (FastBridge) - Realization Gap の解消 (出力を直接力とせずフィルタへ)
%     % 5. D. Mellinger (2011) - 7次フィルタを通した C^6 (Popまで連続) な軌道の生成
% 
%     properties
%         base_ref
%         self
...
%         k_att = 2.0;       % 引力ゲイン
%         k_rep = 15.0;      % 斥力ゲイン
%         d_obs_max = 5.0;   % 斥力が発生し始める距離 (検知範囲)
%         force_max = 5.0;   % 斥力の発散を防ぐ最大力 (Smooth Saturation)
% 
%         % フィルタ パラメータ
%         w_filt = 2.0;
%         k_coeffs
...
%             if isfield(opts, "k_rep"),     obj.k_rep     = opts.k_rep;     end
%             if isfield(opts, "d_obs_max"), obj.d_obs_max = opts.d_obs_max; end
%             if isfield(opts, "force_max"), obj.force_max = opts.force_max; end
%             if isfield(opts, "w_filt"),    obj.w_filt    = opts.w_filt;    end
% 
%             % 7次フィルタの極配置 (C^6 連続性の担保)
...
% 
%             % 2. Khatib (1986): 引力 (Attractive Force) の計算
%             p_nom = xd_nominal(1:3);
%             F_att = obj.k_att * (p_nom - pL_cur);
% 
%             % 3. 障害物情報の取得と斥力 (Repulsive Force) の計算
%             obs_list = [];
%             try
...
%                     if dist_c > 1e-3
%                         u_dir = vec_c / dist_c;
%                         % 楕円体の有効半径 (Funada et al.)
%                         r_obs_dir = 1 / sqrt(max(1e-4, u_dir' * (R_o * diag(1 ./ (radii.^2)) * R_o') * u_dir));
%                         d_surf = dist_c - r_obs_dir - r_sph;
% 
%                         % Khatib (1986): 距離が d_obs_max 以内なら斥力発生
%                         if d_surf > 0 && d_surf < obj.d_obs_max
%                             % F = k * (1/d - 1/d0) * (1/d^2) * \nabla d
%                             force_mag = obj.k_rep * (1/d_surf - 1/obj.d_obs_max) * (1 / (d_surf^2));
% 
%                             % Cohen et al. (Smooth Softplus) 簡易版: 力の発散をサチュレーションで防ぐ
%                             force_mag = min(force_mag, obj.force_max);
% 
%                             F_rep = F_rep + force_mag * u_dir;
%                         end
%                     end
%                 end
%             end
% 
%             % 4. Tscholl et al. (FastBridge) 思想: 
%             % 合力を直接加速度にせず、「仮想の目標位置 (p_target)」に変換する
%             F_total = F_att + F_rep;
%             p_target = pL_cur + F_total; % 係数は k_att に吸収されているため1倍でよい
% 
%             % 5. Mellinger (2011) 準拠: 7次正準系フィルタによる完全 C^6 整形
%             % APFの荒々しい出力を、微分可能な滑らかな軌道に変換する
%             for ax = 1:3
%                 p_curr     = obj.x_int(ax);
...
%             xd(17:19) = obj.x_int(13:15); % s_L
%             xd(21:23) = obj.x_int(16:18); % c_L
%             xd(25:27) = obj.x_int(19:21); % pop_L
% 
