function ref = gen_ref_figure8(param)
% gen_ref_figure8  8の字（Lissajous）軌跡の参照関数
%
...
%       {"gen_ref_figure8", {"freq",10,"orig",[0 0 1],"size",[1 1 0],"phase",0}});
%
...
%
% 8の字パラメトリック式（Lissajous, x/y 周波数比 1:2）:
%   x(t) = lx * sin(w*t + phase)           → 振幅 ±lx
%   y(t) = ly * sin(2*(w*t+phase)) / 2     → 振幅 ±ly/2,  x=0 で y=0 交差
%   ※ y を /2 することで x と同じ freq=10s でも中心交差が成立する
...
px_s = lx * wu * sin(w*t_s + phase)          + lx_offset;
py_s = ly * wu * sin(2*(w*t_s + phase)) / 2  + ly_offset;
...
fprintf("gen_ref_figure8: T=%.1fs  size=[%.2f %.2f %.2f]  orig=[%.2f %.2f %.2f]\n", ...
