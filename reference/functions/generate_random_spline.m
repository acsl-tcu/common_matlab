function [ref, traj] = generate_random_spline(param)
...
arguments
    ...
    param.start (3,1) double {mustBeReal,mustBeFinite} = [0; 0; 1] % 固定する始点 [m]
    ...
    param.coverage (1,1) double {mustBeReal,mustBeFinite,mustBeNonnegative,mustBeLessThan(param.coverage,1)} = 0.9 % 各軸の範囲利用率の下限（0で従来方式）
    param.wiggle (1,1) double {mustBeReal,mustBeFinite,mustBeNonnegative,mustBeLessThanOrEqual(param.wiggle,1)} = 0.8 % 途中のうねりの強さ（0～1、0で追加なし）
...
% 各軸の両端付近へ向かう候補を作り、制約違反時は始点以外を再生成する。
